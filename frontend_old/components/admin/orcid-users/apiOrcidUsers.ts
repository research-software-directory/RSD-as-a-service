...
export async function getOrcidList({page,rows,searchFor,token}:FetchOrcidIds) {
  try{
...
export async function createOrcid({orcid,token}:{orcid:string,token:string}){
  try{
...
export async function deleteOrcid({orcid,token}:{orcid: string,token:string}) {
  try{
...
export async function fetchNameFromOrcid(orcid: string){
  try{
    ...
    logger(`fetchNameFromOrcid: ${resp.status}: ${resp.statusText}`,'warn')
    return null
  }catch(e:any){
...
export async function fetchOrcidPersons(orcids: string[]){
  try{
    ...
    if (newOrcids.length > 0){
      ...
      const response = await fetch(url, {headers: {...createJsonHeaders()}})
...
export function matchOrcidToPerson(orcids:string[], personsMap:Map<string,ExpandedOrcidResult>|undefined){
...
export function useOrcidList(){
  ...
  const {searchFor, page, rows, setCount,setSearchInput} = usePaginationWithSearch('Provide valid ORCID')
