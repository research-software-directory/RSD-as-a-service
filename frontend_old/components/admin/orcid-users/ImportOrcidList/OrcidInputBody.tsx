...
import {OrcidBulkImportReport, useValidateOrcidList} from './apiImportOrcidList'
...
export default function OrcidInputBody({onCancel, onSubmit}: OrcidInputDialogBodyProps) {
    ...
    <>
      ...
      <ImportDialogActions>
        <div className={`flex-1 flex gap-4 text-sm ${error.message ? 'text-error ' : 'text-base-content-disabled'}` }>
          <span>{error.message ? error.message : config.orcidInput.helperText}...{`${error.count}/${config.orcidInput.maxRows}`}</span>
        </div>
        <div>
          <Button
            tabIndex={1}
