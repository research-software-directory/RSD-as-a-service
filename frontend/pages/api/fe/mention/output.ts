 ...
 * @returns MentionItem[]
 */
export async function searchForAvailableOutput({
	project,
	searchFor,
	token,
}: {
...
export async function findPublicationByTitle({
	...
	if (crossref.status === 'fulfilled') {
		...
		logger(
			`output.findPublicationByTitle: Crossref request timeout after ${crossrefTimeoutSec}sec.`,
			'warn',
		);
	}
	...
	if (datacite.status === 'fulfilled') {
		...
		logger(
			`output.findPublicationByTitle: Datacite request failed ${datacite.reason}`,
			'warn',
		);
	}
	...
	if (rsd.status === 'fulfilled') {
		...
		logger(
			`output.findPublicationByTitle: RSD request failed ${rsd.reason}`,
			'warn',
		);
	}
	// return results
