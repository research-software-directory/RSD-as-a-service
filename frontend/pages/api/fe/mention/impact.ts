 ...
 * @returns MentionItem[]
 */
export async function searchForAvailableImpact({
	project,
	searchFor,
	token,
}: {
...
export async function findPublicationByTitle({
	...
	if (crossref.status === 'fulfilled') {
		...
		logger(
			`impact.findPublicationByTitle: Crossref request timeout after ${crossrefTimeoutSec}sec.`,
			'warn',
		);
	}
	...
	if (datacite.status === 'fulfilled') {
		...
		logger(
			`impact.findPublicationByTitle: Datacite request failed ${datacite.reason}`,
			'warn',
		);
	}
	...
	if (rsd.status === 'fulfilled') {
		...
		logger(
			`impact.findPublicationByTitle: RSD request failed ${rsd.reason}`,
			'warn',
		);
	}
	// return results
