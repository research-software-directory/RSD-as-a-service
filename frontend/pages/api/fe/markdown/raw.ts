...
export default async function handler(
  ...
  try {
    ...
    if (url === undefined || Array.isArray(url)) {
      ...
      res.status(200)
      res.json({status: 400, message: 'Missing url as query parameter'})
    }

    // validation 2: check if url starting with http(s)
    if (URL.test(url)===false){
      // we return 200 to api call
      // and the error as json body
      res.status(200)
      res.json({
        status: 400,
        message: 'Not valid url. It should start with http(s)://'
      })
    }
