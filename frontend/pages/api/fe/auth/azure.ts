	...
	if (wellknownUrl) {
		...
		if (authorization_endpoint) {
			...
			const props: RedirectToProps = {
				...
				redirect_uri:
					process.env.AZURE_REDIRECT ??
					'https://research-software.nl/auth/login/azure',
				client_id:
					process.env.AZURE_CLIENT_ID ?? 'www.research-software.nl',
...
export async function azureInfo() {
	...
	if (redirectProps) {
		...
		return {
			...
			redirectUrl,
			html:
				process.env.AZURE_DESCRIPTION_HTML ||
