...
export default function InviteCommunityMaintainer({
	communityInfo,
	error,
}: InviteCommunityMaintainerProps) {
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
	...
	if (id) {
		...
		const resp: InviteCommunityMaintainerProps =
			await claimCommunityMaintainerInvite({
				id: id.toString(),
				token,
			});
	...
	return {
		props: {
			...
			error: {
				status: 404,
				message:
					"This invite is invalid. It's missing invite id. Please ask the community maintainer to provide you a new link.",
			},
		},
	};
}
