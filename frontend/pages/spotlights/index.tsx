...
const pageDesc =
	'The list of research software registered in the Research Software Directory.';
						...
						<div className="flex-1">
							<SoftwareSearchSection
								...
								placeholder={
									...
									:	`Find ${host.software_highlights?.title}`
								}
								layout={view}
								setView={setLayout}
								setModal={setModal}
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
	...
	const {search, keywords, prog_lang, licenses, order, rows, page} =
		ssrSoftwareParams(context.query);
	...
	if (order) {
		...
		const orderInfo = highlightOrderOptions.find(
			item => item.key === order,
		);
	...
	const [highlights, keywordsList, languagesList, licensesList] =
		await Promise.all([
			getSoftwareList({url}),
