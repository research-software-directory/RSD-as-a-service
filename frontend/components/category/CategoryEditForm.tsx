...
export default function CategoryEditForm({
	...
	async function createNewCategory(formData: CategoryEntry) {
		...
		if (!resp.ok) {
			showErrorMessage(
				`Couldn't update category data, we got the following error: ${await resp.text()}`,
			);
		...
		>
			...
			:	<>
					<input
						type="hidden"
						{...register('id', {
							value: data === null ? undefined : data.id,
						})}
					/>
					<input
						type="hidden"
						{...register('parent', {
							value: data === null ? null : data.parent,
						})}
					/>
				</>
			}
			...
			<h3 className="py-4 text-base-content-secondary">
				{getFormTitle()}
			</h3>
				...
				<>
					...
					<TextFieldWithCounter
						...
						options={{
							...
							error:
								formState.errors?.properties?.description
									?.message !== undefined,
						}}
					/>
					<div className="py-2" />
					<TextFieldWithCounter
						...
						options={{
							...
							error:
								formState.errors?.properties?.subtitle
									?.message !== undefined,
						}}
					/>
				</>
