...
function timeRange(data: LineData[]) {
	...
	const range = [new Date(first), new Date(last)];
...
export default function drawLineChart(props: LineChartConfig) {
	...
	const xScale = d3.scaleTime().domain(timeRange(data)).range([0, width]);
	...
	const generateScaledLine = d3
		...
		.y((d: any, i) => {
			...
			if (val < 0) {
				logger(
					'drawLineChart.generateScaledLine...unexpected negative y values',
					'warn',
				);
			}
			return val;
	...
	const lineGroup = group.append('g').attr('id', 'd3-line-group');
