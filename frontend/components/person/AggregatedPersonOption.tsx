...
export default function AggregatedPersonOption({
	option,
}: {
	...
	return (
		<article className="flex-1 flex">
			<ContributorAvatar
				avatarUrl={
					getImageUrl(option.data?.avatar_options[0] ?? null) ?? ''
				}
			...
			<section className="flex-1">
				<div className="grid grid-cols-[3fr,1fr] gap-2">
					...
					<span className="text-right">
						{option.data?.sources.join(', ')}
					</span>
				</div>
				<div className="py-1 text-[0.75rem]">{renderSecondRow()}</div>
			</section>
		</article>
	);
}
