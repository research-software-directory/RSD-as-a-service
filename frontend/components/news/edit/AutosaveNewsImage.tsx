...
export default function AutosaveNewsImage() {
	...
	const {showWarningMessage, showErrorMessage, showInfoMessage} =
		useSnackbar();
	const {watch, setValue} = useFormContext<NewsItem>();

	const [form_id, image_for_news, description] = watch([
		'id',
		'image_for_news',
		'description',
	]);
	...
	async function saveImage(image_b64: string, mime_type: string) {
		...
		// update form values
		setValue('image_for_news', [...image_for_news, add.message]);
	}
	...
	async function removeImage(image: NewsImageProps) {
		...
		if (resp.status === 200 && image.image_id) {
			...
			setValue(
				'image_for_news',
				image_for_news.filter(item => item.id !== image.id),
			);
			...
			if (description.includes(imgLink) === true) {
				...
				showInfoMessage(
					'Removed image and the image link from the markdown.',
				);
			}
		} else {
