...
export async function getNewsItemBySlug({
	...
	try {
		...
		logger(
			`getNewsItemBySlug failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return null;
	} catch (e: any) {
...
export async function getNewsImages({id, token}: {id: string; token?: string}) {
	try {
		...
		logger(
			`getNewsImages failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return [];
	} catch (e: any) {
...
export async function addNewsImage({
	...
	try {
		...
		logger(
			`addNewsImage failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return {
...
export async function getNewsList({
	...
	try {
		...
		const select =
			'id,slug,publication_date,author,is_published,title,summary,image_for_news(id,image_id,position)';
		...
		logger(
			`getNewsList failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return {
			count: 0,
...
export async function validSlugNews({
	date,
	slug,
	token,
}: {
...
export async function getTopNews(items: number) {
	try {
		...
		logger(
			`getTopNews failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return [];
	} catch (e: any) {
...
export function getCardImageUrl(image_for_news: NewsImageProps[]) {
	try {
		if (image_for_news?.length > 0) {
			...
			const cardImage = image_for_news.find(
				item => item.position === 'card',
			);
			if (cardImage) {
