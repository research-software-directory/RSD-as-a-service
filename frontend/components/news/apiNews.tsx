...
export async function getNewsList({page,rows,is_published=true,token,searchFor,orderBy}: GetNewsListProps) {
  try {
    ...
    const select = 'id,slug,publication_date,author,is_published,title,subtitle,image_id'
    // get published meta pages ordered by position
    let query = paginationUrlParams({rows, page})
    if (searchFor) {
      query+=`&or=(title.ilike.*${searchFor}*,subtitle.ilike.*${searchFor}*,author.ilike.*${searchFor}*)`
    }
    if (orderBy) {
      query+=`&order=${orderBy}`
    } else {
