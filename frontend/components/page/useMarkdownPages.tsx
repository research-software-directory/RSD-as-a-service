...
export function useMarkdownPage({slug,token,is_published}:UseMarkdownpageProps) {
  ...
  useEffect(() => {
    async function getPage() {
      setLoading(true)
      const resp = await getMarkdownPage({slug,token,is_published})
      if (resp.page) {
        setPage(resp.page)
      } else {
        setPage(undefined)
    ...
    if (slug) {
      ...
      // we got no slug -> undefined page
      setPage(undefined)
      setLoading(false)
    }
  }, [slug,token,is_published])
