...
export default function MentionEditList({
	title,
	items,
}: MentionSectionListProps) {
	...
	return (
		<AccordionForLightTheme title={title} badgeContent={items.length}>
			<ul>
				{selection.map((item, pos) => {
					return (
						<li
							key={item.id ?? pos}
							className="p-4 hover:bg-base-200 hover:text-base-900"
						>
							<MentionEditItem pos={pos + 1} item={item} />
						</li>
					);
				})}
				{hasMore ?
