...
export default function DoiInputDialogBody({
	onCancel,
	onSubmit,
}: DoiInputDialogBodyProps) {
	...
	return (
		<>
			<ImportDialogTitle title={'Import publications'} />
			<DialogContent
				sx={{
					display: 'flex',
			...
			<ImportDialogActions>
				...
				>
					<span>
						{error.message ?
							error.message
						:	config.doiInput.helperText}
						...{`${error.count}/${config.doiInput.maxRows}`}
					</span>
				</div>
				<div>
					<Button
