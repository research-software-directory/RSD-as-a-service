...
export default function ImportReportBody({
	initialResults,
	onCancel,
	onImport,
}: BulkImportReportBodyProps) {
	...
	function startImport() {
		...
		const selection = searchResults.filter(
			result => result.include && result.mention,
		);
				...
				>
					<ListItemText
						primary={
							result?.mention ?
								<a
									href={`https://doi.org/${result.mention.doi}`}
									target="_blank"
								>
									{result.mention.doi}
								</a>
						...
						secondary={
							result?.mention ?
								<>
									...
									/>
									<br />
									<span>{result.mention.authors}</span>
									<br />
