...
export default function MentionEditSection() {
	...
	function renderHighlights(highlightedMentions: MentionItemProps[]) {
		...
		return (
			<>
				...
				{highlightedMentions
					.sort((a, b) =>
						sortOnNumProp(a, b, 'publication_year', 'desc'),
					)
					.map(item => {
						return (
							<MentionEditFeatured key={item.id} item={item} />
						);
					})}
			</>
		);
	...
	function renderMentionList(mentionByType: MentionByType) {
		...
		return mentionTypes.map(key => {
			...
			// debugger
			return (
				<MentionEditList key={key} title={title} items={items ?? []} />
			);
		});
	}
						...
						<p>
							...
							<strong>
								<SanitizedMathMLBox
									component="span"
									rawHtml={
										confirmModal?.item?.title ?? 'this item'
									}
								/>
							</strong>
							?
