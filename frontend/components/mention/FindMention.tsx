...
export default function FindMention({
	...
	function renderAddOption(
		...
		if (
			...
			return (
				<p key={option.key} className="px-4">
		...
		if (
			option?.label.length < config.minLength ||
			typeof onCreate == 'undefined'
		) {
	...
	function renderOption(
		...
		return (
			<li data-testid="find-mention-option" {...props} key={option.key}>
				<MentionItemBase
					item={option.data}
					nav={
						// we show source on nav position
						<span className="pl-4 text-base-600">
							{option.data.source}
						</span>
					}
