...
export async function getMaintainersOfProject({
	project,
	token,
}: {
	project: string;
	...
	try {
		...
		// ERRORS
		logger(
			`getMaintainersOfProject: ${resp.status}:${resp.statusText} project: ${project}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function deleteMaintainerFromProject({
	maintainer,
	project,
	token,
}: {
