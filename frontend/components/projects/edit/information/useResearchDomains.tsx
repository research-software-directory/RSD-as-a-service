...
export default function useResearchDomains({
	l1Selected,
	l2Selected,
}: UseResearchDomainProps) {
	...
	useEffect(() => {
		...
		async function getResearchDomains() {
			...
			if (resp.status === 200) {
				...
				logger(
					`useResearchDomainOptions.getResearchDomains: ${resp.status}: ${resp.statusText}`,
					'warn',
				);
				if (abort) return;
	...
	useEffect(() => {
		if (l1Selected && domains && domains.children) {
	...
	useEffect(() => {
		if (l2Selected && domains && domains.children) {
