...
export default function AutosaveFundingOrganisations({
	id,
	items,
}: FundingOrganisationProps) {
	...
	async function onAddOrganisation(selected: SearchOrganisation) {
		...
		if (find.length === 0) {
			...
			if (resp.status === 200) {
				const items = [...organisations, selected];
	...
	async function onRemoveOgranisation(pos: number) {
		...
		if (item.id) {
			...
			if (resp.status === 200) {
				...
				showErrorMessage(
					`Failed to delete organisation. ${resp.message}`,
				);
			}
		}
	}
		...
		<>
			...
			<FindFundingOrganisation
				config={{
					freeSolo: false,
					minLength:
						config.funding_organisations.validation.minLength,
