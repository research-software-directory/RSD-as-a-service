...
export default function AutosaveResearchDomains({
	project_id,
	research_domains,
}: ResearchDomainsProps) {
	...
	const {l1Domains, l2Domains, l3Domains} = useResearchDomains({
		l1Selected,
		l2Selected,
	});
	...
	function selectDomain({
		...
		switch (level) {
			...
			default:
				logger(
					`ResearchDomain.selectDomain level ${level} NOT SUPPORTED`,
					'warn',
				);
		}
	}
	...
	async function addDomains() {
		...
		if (resp.status === 200) {
			const newDomains = [...domains, ...toAdd].sort((a, b) =>
	...
	async function onRemove(pos: number) {
		...
		if (item.id) {
			...
			if (resp.status === 200) {
				...
				showErrorMessage(
					`Failed to remove ${item.name}. ${resp.message}`,
				);
			}
		}
	}
		...
		<>
			...
			<div className="flex flex-col mb-4">
				<FormControl
					...
					// }}
				>
					<InputLabel shrink={true}>Level 1</InputLabel>
					<Select
				...
				<FormControl
					...
					// }}
				>
					<InputLabel shrink={true}>Level 2</InputLabel>
				...
				<FormControl variant="standard" fullWidth>
					<InputLabel shrink={true}>Level 3</InputLabel>
