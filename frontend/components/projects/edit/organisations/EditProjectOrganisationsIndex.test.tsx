...
jest.mock('~/utils/getProjects', () => ({
	getOrganisationsOfProject: jest.fn(props =>
		mockGetOrganisationsOfProject(props),
	),
}));

// MOCK deleteOrganisationFromProject
const mockDeleteOrganisationFromProject = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
	}),
...
const mockAddOrganisationToProject = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'approved',
	}),
...
jest.mock('~/utils/editProject', () => ({
	...
	deleteOrganisationFromProject: jest.fn(props =>
		mockDeleteOrganisationFromProject(props),
	),
	patchOrganisationPositions: jest.fn(props =>
		mockPatchOrganisationPositions(props),
	),
	addOrganisationToProject: jest.fn(props =>
		mockAddOrganisationToProject(props),
	),
}));
...
describe('frontend/components/projects/edit/organisations/index.tsx', () => {
	...
	it('renders project organisations', async () => {
		...
		mockGetOrganisationsOfProject.mockResolvedValueOnce(
			mockOrganisationsOfProject as any,
		);
	...
	it('can remove organisation from project', async () => {
		...
		await waitFor(() => {
			...
			expect(remained.length).toEqual(
				mockOrganisationsOfProject.length - 1,
			);
		});
	});
});
