...
export default function RelatedProjectsForProject() {
	...
	const [relatedProject, setRelatedProject] =
		useState<RelatedProjectForProject[]>();
	...
	async function onAdd(selected: SearchProject) {
		...
		if (find.length === 0) {
			...
			if (resp.status !== 200) {
				showErrorMessage(
					`Failed to add related project. ${resp.message}`,
				);
			} else {
				const newList = [
					...relatedProject,
	...
	async function onRemove(pos: number) {
		...
		if (related) {
			...
			if (errors.length > 0) {
				...
				showErrorMessage(
					`Failed to remove related project. ${errors[0].message}`,
				);
			} else {
				const newList = [
