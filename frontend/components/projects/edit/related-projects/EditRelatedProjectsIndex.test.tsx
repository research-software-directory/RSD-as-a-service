...
const mockGetRelatedProjectsForProject = jest.fn(props =>
...
jest.mock('~/utils/getProjects', () => ({
	getRelatedProjectsForProject: jest.fn(props =>
		mockGetRelatedProjectsForProject(props),
	),
...
describe('frontend/components/projects/edit/related-projects/index.tsx', () => {
	...
	it('renders related project items', async () => {
		...
		const relatedProjects = await screen.findAllByTestId(
	...
	it('can add related project to project', async () => {
		...
		// return mocked values
		mockGetRelatedProjectsForProject.mockResolvedValueOnce([]);
		mockSearchForRelatedProjectByTitle.mockResolvedValueOnce(
			relatedProjectsFound,
		);
	...
	it('can remove related project for project', async () => {
		...
		await waitFor(() => {
			...
			const remainedProjects = screen.getAllByTestId(
				'related-project-item',
			);
			expect(remainedProjects.length).toEqual(
				mockRelatedProjects.length - 1,
			);
		});
	});
});
