...
export default function ProjectTeam() {
	...
	const {token, loading, project, members, setMembers} = useTeamMembers({
		slug,
	});
	...
	function updateLocalState(member: TeamMember, pos?: number) {
		if (typeof pos !== 'undefined') {
			...
			// append to bottom
			const newMembersList = [...members, member];
			updateContactPeople(member, newMembersList);
			setMembers(newMembersList);
		}
	}
	...
	function onSubmitMember({
		member,
		pos,
	}: {
		member: SaveTeamMember;
		pos?: number;
	}) {
		// update local state
	...
	async function sortTeamMembers(newList: TeamMember[]) {
		...
		if (newList.length > 0) {
			...
			if (resp.status !== 200) {
				...
				setMembers(members);
				showErrorMessage(
					`Failed to update team positions. ${resp.message}`,
				);
			}
		} else {
			// clear list
				...
				<ConfirmDeleteModal
					...
					body={
						<p>
							...
							<strong>
								{modal.delete.displayName ?? 'No name'}
							</strong>
							?
						</p>
					}
