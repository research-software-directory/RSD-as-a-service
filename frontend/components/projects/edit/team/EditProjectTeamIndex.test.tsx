...
// MOCK postTeamMember
const mockPostTeamMember = jest.fn(props =>
	Promise.resolve({
		status: 201,
		message: 'unique-person-id',
	}),
);
const mockPatchTeamMember = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
	}),
);
const mockDeleteTeamMemberById = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
	}),
);
const mockPatchTeamMemberPositions = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
	}),
...
jest.mock('./editTeamMembers', () => ({
	...
	patchTeamMemberPositions: jest.fn(props =>
		mockPatchTeamMemberPositions(props),
	),
}));

// MOCK deleteImage
const mockDeleteImage = jest.fn(props => Promise.resolve('OK'));
const mockUpsertImage = jest.fn(props =>
	Promise.resolve({
		status: 201,
...
const mockHandleFileUpload = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
...
describe('frontend/components/projects/edit/team/index.tsx', () => {
	...
	it('renders no team members message', async () => {
		...
		const noMembersMsg =
			within(noMembersAlert).getByText('No team members');
	...
	it('can replace avatar image', async () => {
		...
		const imageInput: any = within(modal).getByTestId(
			'upload-avatar-input',
		);
		// set file to upload
