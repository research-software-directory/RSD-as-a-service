...
const mockGetRelatedSoftwareForProject = jest.fn(props =>
	Promise.resolve([] as any),
);
const mockSearchForRelatedProjectByTitle = jest.fn(props =>
	Promise.resolve([] as any),
);
jest.mock('~/utils/getProjects', () => ({
	getRelatedSoftwareForProject: jest.fn(props =>
		mockGetRelatedSoftwareForProject(props),
	),
	searchForRelatedProjectByTitle: jest.fn(props =>
		mockSearchForRelatedProjectByTitle(props),
	),
}));
...
const mockSearchForRelatedSoftware = jest.fn(props =>
	Promise.resolve([] as any),
);
jest.mock('~/utils/editRelatedSoftware', () => ({
	searchForRelatedSoftware: jest.fn(props =>
		mockSearchForRelatedSoftware(props),
	),
}));
...
describe('frontend/components/projects/edit/related-software/index.tsx', () => {
	...
	it('renders related software items', async () => {
		...
		const relatedSoftware = await screen.findAllByTestId(
		...
		const relatedSoftwareFound = [
			{
				id: 'test-id-1',
				slug: 'test-slug-1',
				brand_name: 'Test title 1',
				short_statement: 'Test subtitle 1',
				status: 'approved',
			},
			{
				id: 'test-id-2',
				slug: 'test-slug-2',
				brand_name: 'Test title 2',
				short_statement: 'Test subtitle 2',
				status: 'approved',
			},
		];
		// return mocked values
		mockGetRelatedSoftwareForProject.mockResolvedValueOnce([]);
		mockSearchForRelatedSoftware.mockResolvedValueOnce(
	...
	it('can remove related software', async () => {
		...
		await waitFor(() => {
			...
			const remainedSoftware = screen.getAllByTestId(
				'related-software-item',
			);
			expect(remainedSoftware.length).toEqual(
				mockRelatedSoftware.length - 1,
			);
		});
	});
});
