...
export default function AddProjectCard() {
	...
	const [slug, project_title, project_subtitle] = watch([
		'slug',
		'project_title',
		'project_subtitle',
	]);
				...
				<section className="py-8">
					...
					<TextFieldWithCounter
						options={{
							...
							rows: 3,
							error:
								errors?.project_subtitle?.message !== undefined,
							label: config.project_subtitle.label,
							helperTextMessage:
								errors?.project_subtitle?.message ??
								config.project_subtitle.help,
							helperTextCnt: `${project_subtitle?.length ?? 0}/${config.project_subtitle?.validation?.maxLength?.value ?? 0}`,
							variant: 'outlined',
						}}
						register={register(
							'project_subtitle',
							config.project_subtitle.validation,
						)}
					/>
					<div className="py-4"></div>
					<SlugTextField
