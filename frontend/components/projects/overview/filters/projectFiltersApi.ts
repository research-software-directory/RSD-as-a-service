...
export async function projectKeywordsFilter({
	search,
	keywords,
	domains,
	organisations,
	...
	try {
		...
		logger(
			`projectKeywordsFilter: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function projectDomainsFilter({
	search,
	keywords,
	domains,
	organisations,
	...
	try {
		...
		const domainsOptions = await getDomainsFilterList({
			search,
			keywords,
			domains,
			organisations,
		...
		if (domainsOptions.length > 0) {
			...
			const domainsList = createDomainsList(domainsOptions, domainsInfo);
			return domainsList;
		}
...
export async function getDomainsFilterList({
	search,
	keywords,
	domains,
	organisations,
	...
	try {
		...
		logger(
			`getDomainsFilterList: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function getResearchDomainInfo(keys: string[]) {
	try {
		...
		logger(
			`getResearchDomainInfo: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export function createDomainsList(
	domainOptions: DomainsFilterOption[],
	domainInfo: ResearchDomainInfo[],
) {
	let domainsList: ResearchDomainOption[] = [];
	// return empty list
	if (domainOptions.length === 0 || domainInfo.length === 0)
		return domainsList;
	//
...
export async function projectParticipatingOrganisationsFilter({
	search,
	keywords,
	domains,
	organisations,
	...
	try {
		const query =
			'rpc/project_participating_organisations_filter?order=organisation';
		const url = `${getBaseUrl()}/${query}`;
		const filter = buildProjectFilter({
			search,
			keywords,
		...
		if (resp.status === 200) {
			const json: ParicipatingOrganisationFilterOption[] =
				await resp.json();
			return json;
		}

		logger(
			`projectParticipatingOrganisationsFilter: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
		logger(
			`projectParticipatingOrganisationsFilter: ${e?.message}`,
			'error',
		);
		return [];
	}
...
export async function projectStatusFilter({
	search,
	keywords,
	domains,
	organisations,
	...
	try {
		...
		logger(
			`projectStatusFilter: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
