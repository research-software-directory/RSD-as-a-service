...
export default function RsdUsersList({
	...
	const {loading, accounts, deleteAccount} = useRsdAccounts(
		token,
		adminsOnly,
		inactiveDays,
	);
		...
		return (
			<section className="flex-1">
				<Alert severity="warning" sx={{marginTop: '0.5rem'}}>
					<AlertTitle sx={{fontWeight: 500}}>No users</AlertTitle>A
					user is{' '}
					<strong>automatically added after first login</strong>.
				</Alert>
			</section>
		);
	}
				...
				body={
					<>
						<p>
							...
							<strong>{modal?.account?.id}</strong>?
						</p>
						<p className="mt-4">
							The user will lose all maintainer rights and all
							unused maintainer invites created by this user will
							be deleted.
						</p>
					</>
				}
