...
export async function getSoftwareHighlights({
	limit,
	offset,
	token,
	searchFor,
	...
	try {
		...
		logger(
			`getSoftwareHighlights: ${resp.status}: ${resp.statusText}`,
			'warn',
		);
		return {
			count: 0,
...
export async function addSoftwareHighlight({
	id,
	position,
	token,
}: {
...
export async function patchSoftwareHighlights({
	highlights,
	token,
}: {
...
export async function deleteSoftwareHighlight({
	id,
	token,
}: {
	id: string;
