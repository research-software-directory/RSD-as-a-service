...
export default function SortableHighlightsList({
	highlights,
	loading,
	onSorted,
	onDelete,
}: SoftwareHighlightsListProps) {
		...
		return (
			<Alert severity="warning" sx={{marginTop: '0.5rem'}}>
				<AlertTitle sx={{fontWeight: 500}}>
					No software highlights
				</AlertTitle>
				You can add software to highlight section{' '}
				<strong>using search on the right</strong>.
			</Alert>
		);
	}
				...
				body={
					<>
						<p>
							...
							<strong>{modal?.highlight?.brand_name}</strong> from
							highlights?
						</p>
					</>
				}
