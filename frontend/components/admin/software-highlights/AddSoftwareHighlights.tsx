...
export default function AddSoftwareHighlights({
	onAddSoftware,
	highlights,
}: AddSoftwareHighlightsProps) {
	const [options, setOptions] = useState<
		AutocompleteOption<SoftwareOverviewItemProps>[]
	>([]);
	...
	async function searchSoftware(searchFor: string) {
		...
		const software = itemsNotInReferenceList({
			list: resp.data ?? [],
			referenceList:
				(highlights as any as SoftwareOverviewItemProps[]) ?? [],
			key: 'id',
		});
	...
	function renderOption(
		...
		return (
			<li data-testid="software-option-found" key={option.key} {...props}>
