...
export async function getMarkdownPage(props: UseMarkdownpageProps) {
	try {
		...
		// log error
		logger(
			`fetchMarkdownPage failed: ${resp?.status} ${resp.statusText}`,
			'warn',
		);
		return {
			page: null,
...
export async function getPageLinks({
	...
	try {
		...
		let query =
			'meta_pages?select=id,slug,title,position,is_published&order=position';
		if (is_published) {
			query += '&is_published=eq.true';
		}
		const url = `${baseUrl}/${query}`;
		...
		logger(
			`getPageLinks failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		return [];
	} catch (e: any) {
...
export async function validPageSlug({
	slug,
	token,
}: {
	slug: string;
	...
	const baseUrl = getBaseUrl();
	// get published meta pages ordered by position
	let query = `meta_pages?select=slug,is_published&slug=eq.${slug}`;
	const url = `${baseUrl}/${query}`;
