...
export default function EditMarkdownPages({links}: {links: RsdLink[]}) {
	...
	function onAddPage(page: MarkdownPage) {
		...
		const newLinks = [...navItems, addLink];
	...
	async function patchPositions(newList: RsdLink[]) {
		...
		if (resp.status !== 200) {
			showErrorMessage(
				`Failed to update page positions. ${resp?.message}`,
			);
			// revert back
	...
	return (
		<>
			<PageTitleSticky style={{padding: '1rem 0rem 2rem 0rem'}}>
