...
import {
	addCommunity as addCommunityToRsd,
	deleteCommunityById,
} from './apiCommunities';
...
import {
	CommunityListProps,
	getCommunityList,
} from '~/components/communities/apiCommunities';
...
export function useAdminCommunities() {
		...
		async (data: EditCommunityProps) => {
			try {
				...
				if (resp.status === 200) {
					...
					// show error
					showErrorMessage(
						`Failed to add community. Error: ${resp.message}`,
					);
					return false;
				}
			} catch (e: any) {
				showErrorMessage(
					`Failed to add community. Error: ${e.message}`,
				);
				return false;
			}
	...
	const deleteCommunity = useCallback(
		async (id: string, logo_id: string | null) => {
			// console.log('deleteCommunity...', item)
			const resp = await deleteCommunityById({id, token});
			if (resp.status !== 200) {
				showErrorMessage(
					`Failed to delete community. Error: ${resp.message}`,
				);
			} else {
