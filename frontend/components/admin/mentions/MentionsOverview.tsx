...
import {
	extractSearchTerm,
	SearchTermInfo,
} from '~/components/software/edit/mentions/utils';
...
export default function MentionsOverview() {
	...
	const {searchFor, page, rows, setCount} =
		usePaginationWithSearch('Find mentions');
	...
	function fetchAndSetMentionList(sanitisedSearch: undefined | string): void {
		fetch(
			`/api/v1/mention?${createSearchQueryParameters(sanitisedSearch)}&${paginationUrlParams({rows, page})}`,
			{
				headers: {
					Prefer: 'count=exact',
				},
	...
	function createSearchQueryParameters(
		sanitisedSearch: undefined | string,
	): string {
		...
		const searchTypeTerm: SearchTermInfo =
			extractSearchTerm(sanitisedSearch);
	...
	return (
		<section className="flex-1">
			...
			<div>
				<MentionsOverviewList
					list={mentionList}
					onUpdate={() => fetchAndSetMentionList(sanitisedSearch)}
				></MentionsOverviewList>
			</div>
		</section>
	);
