...
import {OrcidBulkImportReport, OrcidSearchResult, RsdOrcid} from './apiImportOrcidList'
...
export default function OrcidImportReport({validationReport,onCancel,onImport}: OrcidImportReportProps) {
  ...
  useEffect(() => {
    ...
    if (validationReport) {
      ...
      validationReport.forEach((result) => {
        if (result?.status === 'valid' && result?.include === true) selectedCnt++
      })
    }
    // debugger
      ...
      html.push(
        <ListItem
          ...
          secondaryAction={
            <Switch
              ...
              disabled={(item?.status !== 'valid' && item?.status!=='orcidNotFound')}
        ...
        >
          <ListItemText
            ...
            secondary={
              item?.status==='valid' ?
                <OrcidInfo loading={false} person={item?.data} />
                : <span className="text-error">{generateErrorMessage(item)}</span>
            }
          />
        </ListItem>
      )
