...
export function OrcidInfo({
	...
	if (person['institution-name'] && person['institution-name']?.length > 0) {
		return (
			<>
				<span>{`${person['given-names'] ?? ''} ${person['family-names'] ?? ''}`}</span>
				<br />
				<span>{`${person['institution-name']?.join('; ')}`}</span>
				<br />
			</>
		);
	}
...
export default function OrcidUserItem({
	orcid,
	person,
	removeOrcid,
}: OrcidUserItemProps) {
	return (
		<ListItem
			...
			disableGutters
			secondaryAction={
				<IconButton onClick={() => removeOrcid(orcid)}>
					<DeleteIcon />
				</IconButton>
			}
			sx={{
		...
		>
			<ListItemText
				...
				secondary={<OrcidInfo loading={false} person={person} />}
			/>
		</ListItem>
	);
}
