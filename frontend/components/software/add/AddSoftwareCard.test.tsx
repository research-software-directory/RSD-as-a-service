...
const mockAddSoftware = jest.fn(props =>
...
jest.mock('~/utils/editSoftware', () => {
	return {
		...
		validSoftwareItem: jest.fn((slug, token) =>
			mockValidSoftwareItem(slug, token),
		),
	};
});
...
it('validate, save and redirect', async () => {
	...
	await waitFor(() => {
		...
		expect(mockValidSoftwareItem).toHaveBeenCalledWith(
			slug,
			mockSession.token,
		);
	});
