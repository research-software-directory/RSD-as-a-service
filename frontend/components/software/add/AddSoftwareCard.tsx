...
export default function AddSoftwareCard() {
	...
	const {register, handleSubmit, watch, formState, setValue, setError} =
		useForm<AddSoftwareForm>({
			mode: 'onChange',
		});
	const {errors, isValid} = formState;
	// watch for data change in the form
	const [slug, brand_name, short_statement] = watch([
		'slug',
		'brand_name',
		'short_statement',
	]);
				...
				<section className="py-8">
					...
					<TextFieldWithCounter
						options={{
							...
							rows: 5,
							error:
								errors?.short_statement?.message !== undefined,
						...
						register={register(
							'short_statement',
							config.short_statement.validation,
						)}
					/>
					<div className="py-4"></div>
					<SlugTextField
