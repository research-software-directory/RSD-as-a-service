...
export async function getMaintainersOfSoftware({
	software,
	token,
}: {
	software: string;
	...
	try {
		...
		// ERRORS
		logger(
			`getMaintainersOfSoftware: ${resp.status}:${resp.statusText} software: ${software}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function deleteMaintainerFromSoftware({
	maintainer,
	software,
	token,
}: {
...
export async function softwareMaintainerLink({
	software,
	account,
	token,
}: {
