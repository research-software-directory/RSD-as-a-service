...
import {
	addRelatedSoftware,
	deleteRelatedSoftware,
	getRelatedSoftwareForSoftware,
} from '~/utils/editRelatedSoftware';
...
export default function RelatedSoftwareForSoftware() {
	...
	const [relatedSoftware, setRelatedSoftware] =
		useState<RelatedSoftwareOfSoftware[]>();
	...
	async function onAdd(selected: SearchSoftware) {
		...
		const find = relatedSoftware.filter(
		...
		if (find.length === 0) {
			...
			if (resp.status !== 200) {
				showErrorMessage(
					`Failed to add related software. ${resp.message}`,
				);
			} else {
				const newList = [
					...relatedSoftware,
