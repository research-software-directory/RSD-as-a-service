...
import AvatarOptionsPerson, {
	RequiredAvatarProps,
} from '~/components/person/AvatarOptionsPerson';
...
export default function AggregatedContributorModal({
	open,
	onCancel,
	onSubmit,
	contributor,
}: AggregatedContributorModalProps) {
	const {token} = useSession();
	const {showErrorMessage} = useSnackbar();
	const smallScreen = useMediaQuery('(max-width:600px)');
	const {handleSubmit, watch, formState, reset, control, register, setValue} =
		useForm<NewRsdContributor>({
			mode: 'onChange',
			defaultValues: {
				...contributor,
			},
	...
	async function onSave(data: NewRsdContributor) {
		...
		const contributor: SaveContributor = getPropsFromObject(
			data,
			ContributorProps,
		);
	...
	return (
		...
		>
			...
			>
				{/* hidden inputs */}
				<input type="hidden" {...register('software')} />
				<input type="hidden" {...register('position')} />
				<input type="hidden" {...register('avatar_b64')} />
				<input type="hidden" {...register('avatar_mime_type')} />
				<DialogContent
					sx={{
						width: ['100%', '37rem'],
					}}
				>
					<AvatarOptionsPerson
						watch={
							watch as unknown as UseFormWatch<RequiredAvatarProps>
						}
						setValue={
							setValue as unknown as UseFormSetValue<RequiredAvatarProps>
						}
