	...
	async function onSave(data: Contributor) {
		...
		if (data.avatar_b64 && data.avatar_mime_type) {
			...
			if (upload.status === 201) {
				...
				(data.avatar_b64 = null), (data.avatar_mime_type = null);
			} else {
		...
		if (contributor?.id && typeof pos !== 'undefined') {
			...
			if (resp.status === 200) {
				...
				showErrorMessage(
					`Failed to update ${getDisplayName(data)}. Error: ${resp.message}`,
				);
			}
		} else {
						...
						<div>
							...
							>
								<ContributorAvatar
									...
									avatarUrl={
										formData.avatar_b64 ??
										getImageUrl(formData.avatar_id) ??
										''
									}
									displayName={
									...
									displayInitials={
										getDisplayInitials(contributor ?? {}) ??
										''
									}
								/>
							</label>
