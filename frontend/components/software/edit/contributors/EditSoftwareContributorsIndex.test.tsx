...
const mockGetContributorsForSoftware = jest.fn(props =>
...
jest.mock('~/utils/editContributors', () => ({
	...
	getContributorsForSoftware: jest.fn(props =>
		mockGetContributorsForSoftware(props),
	),
...
jest.mock('~/utils/getInfoFromDatacite', () => ({
	getContributorsFromDoi: jest.fn((...props) =>
		mockGetContributorsFromDoi(props),
	),
}));
...
describe('frontend/components/software/edit/contributors/index.tsx', () => {
	...
	it('renders no contributors message', async () => {
		...
		const noContributorsMsg =
			within(noContributorsAlert).getByText('No contributors');
