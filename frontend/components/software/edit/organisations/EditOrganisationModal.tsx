...
import ControlledImageInput, {
	FormInputsForImage,
} from '~/components/form/ControlledImageInput';
...
export default function EditOrganisationModal({
	...
	return (
		...
		>
			...
			<form
				id={formId}
				onSubmit={handleSubmit((data: EditOrganisation) =>
					onSubmit({data, pos}),
				)}
				autoComplete="off"
			>
				...
				<input type="hidden" {...register('logo_id')} />
				<input type="hidden" {...register('logo_b64')} />
				<input type="hidden" {...register('logo_mime_type')} />
				<DialogContent
					sx={{
						width: ['100%', '37rem'],
				...
				>
					<section className="grid grid-cols-[1fr,3fr] gap-8">
						<ControlledImageInput
							...
							setValue={
								setValue as unknown as UseFormSetValue<FormInputsForImage>
							}
						/>
						<div>
							<ControlledTextField
					...
					<Alert severity="info">
						...
						You are the first to reference this organisation and can
						add a logo now. After clicking on &quot;Save&quot;,
						logos can only be added by organisation maintainers.
					</Alert>
				</DialogContent>
				<DialogActions
					sx={{
