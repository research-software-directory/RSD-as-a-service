...
export default function SoftwareOrganisations() {
	...
	const {loading, organisations, setOrganisations} =
		useParticipatingOrganisations({
	...
	async function onAddOrganisation(item: SearchOrganisation) {
		...
		if (item.ror_id && found) {
			showInfoMessage(
				`${item.name} is already in the collection (based on ROR ID).`,
			);
			return;
		}
		// add default values
		...
		if (item.source === 'ROR') {
			...
			if (resp.status === 200) {
				...
				addOrganisation.status =
					resp.message as SoftwareForOrganisation['status'];
		...
		try {
			...
			if (data.logo_b64 && data.logo_mime_type) {
				...
				if (upload.status === 201) {
					...
					showErrorMessage(
						`Failed to upload image. ${upload.message}`,
					);
					return;
				}
			}
			if (typeof pos !== 'undefined' && data.id) {
				...
				const {status, message} =
					await createOrganisationAndAddToSoftware({
						item: data,
						software: software?.id ?? '',
						token,
					});
	...
	function addOrganisationToList(newItem: EditOrganisation) {
		const newList = [...organisations, newItem];
	...
	async function sortedOrganisations(newList: EditOrganisation[]) {
		if (newList.length > 0) {
			...
			if (resp.status !== 200) {
				...
				// show error
				showErrorMessage(
					`Failed to update organisation positions. ${resp.message}`,
				);
			}
		} else {
			// reset list
