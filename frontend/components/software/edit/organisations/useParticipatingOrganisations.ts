...
async function getParticipatingOrganisationsForSoftware({
	software,
	token,
	account,
...
export function useParticipatingOrganisations({
	software,
	token,
	account,
}: UseParticipatingOrganisationsProps) {
	...
	useEffect(() => {
		...
		async function getOrganisations(
			props: UseParticipatingOrganisationsProps,
		) {
			const organisations =
				await getParticipatingOrganisationsForSoftware(props);
		...
		if (software && token && account && software !== loadedSoftware) {
			getOrganisations({
				software,
				token,
				account,
