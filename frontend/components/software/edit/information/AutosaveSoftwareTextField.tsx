...
export default function AutosaveSoftwareTextField({
	software_id,
	options,
	rules,
}: AutosaveSoftwareInfoProps) {
	const router = useRouter();
	const {token} = useSession();
	const {showErrorMessage} = useSnackbar();
	const {setSoftwareTitle, setSoftwareSlug, setConceptDoi} =
		useSoftwareContext();
	const {control, resetField} = useFormContext();

	async function saveSoftwareInfo({
		name,
		value,
		...
		if (resp?.status !== 200) {
			showErrorMessage(
				`Failed to save ${options.name}. ${resp?.message}`,
			);
		} else {
			// debugger
