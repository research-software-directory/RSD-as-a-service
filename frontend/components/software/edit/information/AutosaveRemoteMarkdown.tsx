...
export default function AutosaveRemoteMarkdown({
	control,
	rules,
	options,
	onSaveField,
}: AutosaveRemoteMarkdownProps) {
	// watch for change
	const markdownUrl = useWatch({control, name: options.name});
	const {errors} = useFormState({control});
	const debouncedUrl = useDebounceValid(
		markdownUrl,
		errors[options.name],
		1000,
	);
	...
	return (
		<div className="pt-4">
			...
			/>
			<h2 className="py-4 text-sm font-medium text-primary tracking-[0.125rem]">
				Preview
			</h2>
			<div className="border rounded-sm min-h-[34rem] flex">
				{renderContent()}
			</div>
		</div>
	);
