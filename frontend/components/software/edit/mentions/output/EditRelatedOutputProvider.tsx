...
import NoRelatedOutputItems from './NoRelatedOutputItems';
import {
...
const initalState: EditMentionState = {
	settings: {
		...
		noItemsComponent: () => <NoRelatedOutputItems />,
	},
	loading: true,
	processing: false,
	mentions: [],
...
function createSuccessMessage(item: MentionItemProps) {
	...
	return <SanitizedMathMLBox component="span" rawHtml={message} />;
}
...
export default function EditRelatedOutputProvider(props: any) {
	...
	const {showErrorMessage, showSuccessMessage, showInfoMessage} =
		useSnackbar();
	const {
		loading,
		output: mentions,
	...
	// console.group('EditRelatedOutputProvider')
	...
	async function processOnSubmit(action: EditMentionAction) {
		...
		if (item.id === null || item.id === '') {
			...
			if (resp.status !== 200) {
				showErrorMessage(
					createErrorMessage(
						`Failed to update ${item.title}. ${resp.message}`,
					),
				);
				return;
			}
	...
	async function processOnAdd(action: EditMentionAction) {
		...
		if (item.doi) {
			const found = state.mentions.find(
				mention => mention.doi === item.doi,
			);
			if (found) {
				showInfoMessage(
					`Mention with DOI ${item.doi} is already in ${getMentionType(item.mention_type, 'plural')}.`,
				);
				return true;
			}
		}
		if (item.id) {
			...
			if (resp.status !== 200) {
				showErrorMessage(
					`Failed to add ${item.title}. ${resp.message}`,
				);
			} else {
				dispatch({
					type: EditMentionActionType.ADD_ITEM,
	...
	async function processOnDelete(action: EditMentionAction) {
		...
		if (item.id) {
			...
			if (resp.status == 200) {
				...
				showErrorMessage(
					createErrorMessage(
						`Failed to delete ${item.title}. ${resp.message}`,
					),
				);
			}
		} else {
			showErrorMessage(
				createErrorMessage(
					`Failed to delete ${item.title}. Invalid item id ${item.id}`,
				),
			);
		}
	}
