...
export default function AutosaveSoftwareLicenses() {
	...
	useEffect(() => {
		...
		if (allOptions.length > 0) {
			...
			const selection = allOptions.filter(option => {
				return (
					keyList.includes(option.key) &&
					option.data.deprecated === false
				);
			});
	...
	function searchLicense(searchFor: string) {
		...
		const found = allOptions.filter(item => {
			...
			return (
				item.label
					.toLowerCase()
					.includes(searchFor.trim().toLowerCase()) ||
				item.key.toLowerCase().includes(searchFor.trim().toLowerCase())
			);
		});
		const sorted = found.sort((a, b) =>
			sortBySearchFor(a, b, 'label', searchFor),
		);
		// debugger
	...
	async function onAddLicense(selected: AutocompleteOption<License>) {
		...
		if (find.length === 0) {
			...
			if (resp.status === 201) {
				...
				const newList = [...licenses, selected];
	...
	function onCreateLicense(newInputValue: string) {
		...
		const find = licenses.filter(
			item => item?.label?.toLowerCase() === newLicense.toLowerCase(),
		);
		// if not found
		if (find.length === 0) {
	...
	function renderOption(
		...
		if (option?.input) {
			...
			if (
				option?.input.length >=
				config.licenses.modal.license.validation.minLength.value
			) {
			...
			<li {...props} key={option.key} title={option.label}>
				...
				{option.data?.deprecated ?
					<span className="text-warning">
						{option.key} ({option.label}) - DEPRECATED
					</span>
				:	<span>
						{option.key}{' '}
						<span className="text-base-content-disabled">
							({option.label})
						</span>
					</span>
				}
		...
		<>
			...
			<div className="flex flex-wrap py-2">
				{licenses.map((item, pos) => {
					return (
						<div key={item.key} className="py-1 pr-1">
							<Chip
