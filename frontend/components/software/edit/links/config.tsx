...
export const config = {
	...
	repository_url: {
		...
		help: (repoUrl: string | null) =>
			repoUrl ? <VerifyGithubUrl url={repoUrl} /> : '',
		validation: {
			maxLength: {value: 200, message: 'Maximum length is 200'},
			pattern: {
				value: /^https?:\/\/.+\..+/,
				message:
					'URL should start with http(s)://, have at least one dot (.) and at least one slash (/).',
			},
		},
	},
	repository_platform: {
	...
	concept_doi: {
		title: 'Software DOI',
		subtitle:
			'Provide the DOI of your software. This DOI will be used to import metadata about the software.',
		label: 'Software DOI',
		help: '',
		...
		validation: {
			...
			pattern: {
				value: /^10(\.\w+)+\/\S+$/,
				message:
					'Invalid DOI pattern. Maybe you provided a complete URL?',
			},
		},
	},
	validateConceptDoi: {
	...
	importKeywords: {
		...
		message: (doi: string) =>
			`Import keywords from datacite.org using DOI ${doi}`,
	},
	licenses: {
		title: 'Licenses',
			...
			license: {
				...
				validation: {
					...
					pattern: {
						...
						message:
							'Restricted input violation. Use letters, numbers, dashes "-" and dot "." only.',
					},
				},
			},
			name: {
	...
	importLicenses: {
		...
		message: (doi: string) =>
			`Import licenses from datacite.org using DOI ${doi}`,
	},
};
