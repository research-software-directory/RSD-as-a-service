...
const licenseOptions: AutocompleteOption<License>[] = licenseForSoftware.map(
	item => ({
		key: item.license,
...
const mockAddLicensesForSoftware = jest.fn(props =>
	Promise.resolve({
		status: 201,
...
it('can add NEW license', async () => {
	...
	const urlInput = screen.getByRole('textbox', {
		name: config.licenses.modal.reference.label,
	});
...
it('can import license from DOI', async () => {
	...
	await waitFor(() => {
		...
		expect(mockGetLicensesFromDoi).toBeCalledWith(
			defaultValues.concept_doi,
		);
