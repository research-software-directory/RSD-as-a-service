...
function prepareLicenses(rawLicense: LicenseForSoftware[] = []) {
	const license: AutocompleteOption<License>[] = rawLicense?.map(
		(item: any) => {
			return {
				key: item.license,
				label: item.name,
...
export async function getSoftwareInfoForEdit({
	slug,
	token,
}: {
	slug: string;
	...
	if (software) {
		...
		// other api requests
		const [keywords, categories, categoryForSoftwareIds, respLicense] =
...
export default function useSoftwareToEdit({
	slug,
	token,
}: {
	slug: string | null;
