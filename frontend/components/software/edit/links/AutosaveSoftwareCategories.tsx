...
export default function AutosaveSoftwareCategories({
	softwareId,
	reorderedCategories,
	associatedCategoryIds,
}: Readonly<SoftwareCategoriesProps>) {
	// trick to force rerender
	const [_, setAssociatedCategories] = useState<Set<CategoryID>>(
		associatedCategoryIds,
	);
	const {token} = useSession();
	const selectedNodes: TreeNode<CategoryEntry>[] = [];
	for (const root of reorderedCategories.all) {
		const rootSelectedSubTree = root.subTreeWhereLeavesSatisfy(value =>
			associatedCategoryIds.has(value.id),
		);
	...
	const availableCategoriesTree: TreeNode<CategoryEntry>[] = useMemo(() => {
		...
		const result =
			generalCategories.childrenCount() > 0 ?
				[...reorderedCategories.highlighted, generalCategories]
	...
	function extractSelectedRoots(
		root: TreeNode<CategoryEntry>,
			...
			{availableCategoriesTree.map(root => {
				...
				return (
					<Fragment key={rootValue.id}>
						...
						<CategoryTreeLevel
							showLongNames
							items={extractSelectedRoots(root)}
