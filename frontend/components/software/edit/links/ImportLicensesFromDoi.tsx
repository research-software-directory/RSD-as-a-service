...
export default function ImportLicensesFromDoi({
	...
	async function importLicensesFromDoi() {
		...
		for (const license of licenses) {
			...
			let spdx = allOptions.find(
				item => item.key.toLowerCase() === license.key?.toLowerCase(),
			);
			if (typeof spdx == 'undefined') {
				// if not found by identifier try to find it by name
				spdx = allOptions.find(
					item =>
			...
			if (typeof spdx !== 'undefined') {
				...
				find = items.find(
					item => item.key.toLowerCase() === spdx?.key.toLowerCase(),
				);
			} else {
				// exclude if already in fields based on key
				find = items.find(
					item =>
						item.key.toLowerCase() === license.key?.toLowerCase(),
				);
		...
		if (importedLicenses.length > 0) {
			...
			const newList = [...items, ...importedLicenses].sort((a, b) =>
				sortOnStrProp(a, b, 'key'),
			);
			// update collection
			...
			if (importedLicenses.length === 1) {
				showSuccessMessage(
					`${importedLicenses.length} license imported from DOI ${concept_doi}`,
				);
			} else if (importedLicenses.length > 1) {
				showSuccessMessage(
					`${importedLicenses.length} licenses imported from DOI ${concept_doi}`,
				);
			}
		} else {
			showInfoMessage(
				`No (additional) license to import from DOI ${concept_doi}`,
			);
		}
		// we only log failed for now
		if (failedLicenses.length > 0) {
			logger(
				`Failed to import licenses [${failedLicenses.toString()}] from DOI [${concept_doi}]`,
				'warn',
			);
		}
		setLoading(false);
