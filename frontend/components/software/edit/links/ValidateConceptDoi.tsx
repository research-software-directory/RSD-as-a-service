...
export default function ValidateConceptDoi({
	doi,
	onUpdate,
	disabled,
}: ValidateConceptDoiProps) {
	...
	async function validateDoi() {
		...
		if (info?.status === 200) {
			...
			showErrorMessage(
				`Failed to retrieve info for DOI: ${doi}. ${info?.message ?? ''}`,
			);
		}
		setLoading(false);
	}
	...
	function extractConceptDoi(dataciteWork: DataciteWorkType) {
		for (const relatedIdentifier of dataciteWork.relatedIdentifiers) {
			if (
				...
				return relatedIdentifier.relatedIdentifier;
		}
		return null;
	}
