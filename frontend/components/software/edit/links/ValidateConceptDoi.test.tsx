...
const mockGetSoftwareVersionInfoForDoi = jest.fn(props =>
	Promise.resolve({
		status: 200,
		data: {
			software: {
...
jest.mock('~/utils/getDataCite', () => ({
	getSoftwareVersionInfoForDoi: jest.fn(props =>
		mockGetSoftwareVersionInfoForDoi(props),
	),
}));
...
it('disabled button when disabled=true ', () => {
	...
	// render
	render(<ValidateConceptDoi {...mockProps} />);
...
it('shows valid concept DOI message', async () => {
				...
				relatedIdentifiers: [
					{
						...
						relatedIdentifier:
							'https://github.com/UtrechtUniversity/animal-sounds/tree/v0.0.1-alpha',
					},
					{
	...
	await waitForElementToBeRemoved(
		within(validateBtn).getByRole('progressbar'),
	);
	// valid DOI message shown
	const validDOI = screen.getByText(
		`The DOI ${mockProps.doi} is a valid Concept DOI`,
	);
});
