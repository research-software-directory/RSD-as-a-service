...
export default function ImportKeywordsFromDoi({
	software_id,
	concept_doi,
	keywords,
	onSetKeywords,
	...
	async function importKeywordsFromDoi() {
		...
		if (importedKeywords.length > 0) {
			...
			const items = [...keywords, ...importedKeywords].sort((a, b) =>
				sortOnStrProp(a, b, 'keyword'),
			);
			// update collection
			onSetKeywords(items);
			showSuccessMessage(
				`${importedKeywords.length} keywords imported from DOI ${concept_doi}`,
			);
		} else if (failedKeywords.length > 0) {
			showInfoMessage(
				`Failed to import keywords [${failedKeywords.toString()}] from DOI [${concept_doi}]`,
			);
			// log failure
			logger(
				`importKeywordsFromDoi: Failed to import keywords [${failedKeywords.toString()}] from DOI [${concept_doi}]`,
				'warn',
			);
		} else {
			showInfoMessage(
				`No (additional) keywords imported from DOI ${concept_doi}`,
			);
		}
		setLoading(false);
	}
