...
import {
	addToRepositoryTable,
	deleteFromRepositoryTable,
} from '~/utils/editSoftware';
...
import AutosaveControlledTextField, {
	OnSaveProps,
} from '~/components/form/AutosaveControlledTextField';
...
async function suggestPlatform(repositoryUrl: string | null) {
	...
	try {
		...
		const resp = await fetch(`${baseUrl}/rpc/suggest_platform`, {
			method: 'POST',
...
export default function AutosaveRepositoryUrl() {
	...
	const {
		...
		field: {value: repository_url},
	} = useController({
		control,
		name: 'repository_url',
	});
	const [id, repository_platform, scraping_disabled_reason] = watch([
		'id',
		'repository_platform',
	...
	useEffect(() => {
		if (typeof urlError == 'undefined' && repository_url) {
			...
			// debugger
			if (platform.id === null) {
				suggestPlatform(repository_url).then(suggestion => {
	...
	async function saveScrapingDisabledReason({value}: {value: string | null}) {
		try {
			const resp = await fetch(
				`/api/v1/repository_url?software=eq.${id}`,
				{
					method: 'PATCH',
			...
			if (!resp.ok) {
				showErrorMessage(
					`Failed to save the disabling reason with status code ${resp.status} and body ${JSON.stringify(resp.body)}`,
				);
			}
		} catch (e) {
			showErrorMessage(
				`Failed to save the disabling reason with an unknown error: ${e}`,
			);
		}
	}

	async function saveRepositoryInfo({
		name,
		value,
	}: OnSaveProps<EditSoftwareItem>) {
		...
		if (resp.status !== 200) {
			...
			resetField('repository_platform', {
				defaultValue: data.code_platform,
			});
		}
	}
			...
			{user?.role === 'rsd_admin' ?
				<AutosaveControlledTextField
					options={{
						...
						helperTextMessage:
							config.repository_url.help(repository_url),
						helperTextCnt: `${scraping_disabled_reason?.length || 0}/200`,
						disabled: user?.role !== 'rsd_admin',
					}}
					control={control}
					rules={
						config.repository_disabled_scraping_reason.validation
					}
