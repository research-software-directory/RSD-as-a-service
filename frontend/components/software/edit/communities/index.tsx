...
export default function SoftwareCommunities() {
	...
	const {loading, communities, joinCommunity, leaveCommunity} =
		useSoftwareCommunities(software.id);
	...
	const [openCategoryModalProps, setOpenCategoryModalProps] = useState<{
		autoConfirm: boolean;
		onSave: (community: CommunityListProps) => void;
	} | null>(null);
	const [selectedCommunity, setSelectedCommunity] =
		useState<CommunityListProps | null>(null);
	...
	function onAddCommunity(community: CommunityListProps) {
		...
		setOpenCategoryModalProps({
			autoConfirm: true,
			onSave: onConfirmAddCommunity,
		});
	}
		...
		<>
			<EditSection className="flex-1 md:flex md:flex-col-reverse md:justify-end xl:grid xl:grid-cols-[3fr,2fr] xl:px-0 xl:gap-[3rem]">
				...
				<section className="py-4">
					...
					/>
					<FindCommunity onAdd={onAddCommunity} />
				...
				<ConfirmDeleteModal
					...
					body={
						<p>
							...
							<strong>{modal.name ?? ''}</strong>? This will also
							delete all related (if any) categories.
						</p>
					}
					onCancel={() =>
