...
import {
	CommunitiesOfSoftware,
	getCommunitiesForSoftware,
	removeCommunityCategoriesFromSoftware,
	removeSoftwareFromCommunity,
...
export function useSoftwareCommunities(software: string) {
	...
	const joinCommunity = useCallback(
		async ({
			...
			if (resp.status === 200) {
				...
				showErrorMessage(
					`Failed to join community ${community.name}. ${resp.message}`,
				);
			}
	...
	const leaveCommunity = useCallback(
		async ({
			...
			const removeCategoriesError =
				await removeCommunityCategoriesFromSoftware(
					software,
					community,
					token,
				);
			if (removeCategoriesError) {
				showErrorMessage(
					`Failed to leave remove community categories: ${removeCategoriesError}`,
				);
				return;
			}
