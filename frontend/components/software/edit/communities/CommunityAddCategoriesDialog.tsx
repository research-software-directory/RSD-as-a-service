...
export default function CommunityAddCategoriesDialog({
	...
	const [categories, setCategories] = useState<
		TreeNode<CategoryEntry>[] | null
	>(null);
	const [error, setError] = useState<string | null>(null);
	const [state, setState] = useState<
		'loading' | 'error' | 'ready' | 'saving'
	>('loading');
	const [selectedCategoryIds, setSelectedCategoryIds] =
		useState<CategoryForSoftwareIds>(new Set());
	...
	useEffect(() => {
		setState('loading');
		const promiseLoadRoots = loadCategoryRoots(community.id).then(roots => {
		...
		const promiseLoadAssociatedCategories = getCategoryForSoftwareIds(
			softwareId,
			token,
	...
	function isSaveDisabled() {
		return (
			categories === null || categories.length === 0 || state !== 'ready'
		);
	}
	...
	async function saveCategoriesAndCommunity() {
		...
		const deleteErrorMessage = await removeCommunityCategoriesFromSoftware(
			softwareId,
			community.id,
			token,
		);
		if (deleteErrorMessage !== null) {
			setError(
				`Couldn't delete the existing categories: ${deleteErrorMessage}`,
			);
			setState('error');
			return;
		}
		...
		const categoriesArrayToSave: {
			software_id: string;
			category_id: string;
		}[] = [];
		selectedCategoryIds.forEach(id => {
			if (availableCategoryIds.has(id)) {
				categoriesArrayToSave.push({
					...
					<>
						{categories === null || categories.length === 0 ?
							<Alert severity="info" sx={{padding: '2rem'}}>
								This community doesn&lsquo;t have any
								categories.
							</Alert>
	...
	return (
		<Dialog open fullScreen={smallScreen}>
			...
			>
				Add categories of {community.name}
			</DialogTitle>
			<DialogContent
				sx={{
					display: 'flex',
