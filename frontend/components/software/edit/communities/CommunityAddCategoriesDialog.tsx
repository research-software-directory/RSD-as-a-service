...
export default function CommunityAddCategoriesDialog({softwareId, community, onClose, onConfirm}: communityAddCategoriesDialogProps) {
  ...
  const isSelected = (node: TreeNode<CategoryEntry>) => {
    const val = node.getValue()
    return val === null ? false : selectedCategoryIds.has(val.id)
  }
  const textExtractor = (value: CategoryEntry) => value.name
  const keyExtractor = (value: CategoryEntry) => value.id
  const onSelect = (node: TreeNode<CategoryEntry>) => {
    const val = node.getValue()
    if (val === null) {
      return
    }
