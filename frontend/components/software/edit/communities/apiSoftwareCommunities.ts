...
export async function getCommunitiesForSoftware({
	software,
	token,
}: {
	software: string;
	...
	try {
		...
		logger(
			`getCommunitiesForSoftware: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function removeCommunityCategoriesFromSoftware(
	softwareId: string,
	communityId: string,
	token: string,
): Promise<string | null> {
...
export async function removeSoftwareFromCommunity({
	software,
	community,
	token,
}: RequestToJoinCommunity) {
