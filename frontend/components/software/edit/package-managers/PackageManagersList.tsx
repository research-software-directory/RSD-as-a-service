...
export default function PackageManagersList({
	loading,
	managers,
	onSorted,
	onEdit,
	onDelete,
}: PackageManagersListProps) {
	...
	if (managers.length === 0) {
		return (
			...
			>
				...
				Please provide <strong>download locations</strong> of your
				software using Add button.
			</Alert>
		);
	}
		...
		<>
			...
			{modal && (
				<ConfirmDeleteModal
					...
					body={
						<p>
							Are you sure you want to remove installation
							location <strong>{modal.location ?? ''}</strong>?
						</p>
					}
					onCancel={() => {
						setModal(undefined);
