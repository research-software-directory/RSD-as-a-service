...
export async function getPackageManagers({
	...
	try {
		...
		logger(
			`getPackageManagers...${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function postPackageManager({
	data,
	token,
}: {
	data: NewPackageManager;
	token: string;
}) {
	try {
...
export async function patchPackageManagers({
	items,
	token,
}: {
...
async function patchPackageManagerItem({
	id,
	key,
	value,
	token,
...
export async function deletePackageManager({
	id,
	token,
}: {
	id: string;
...
export async function getPackageManagerTypeFromUrl(url: string) {
	try {
		...
		const resp = await fetch(`${getBaseUrl()}/rpc/suggest_platform`, {
			method: 'POST',
