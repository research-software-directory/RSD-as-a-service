...
async function getSoftwareServices(id: string, token: string) {
	try {
		const select =
			'select=software,url,code_platform,basic_data_scraped_at,basic_data_last_error,languages_scraped_at,languages_last_error,commit_history_scraped_at,commit_history_last_error,scraping_disabled_reason';
		...
		logger(
			`getSoftwareServices...${resp.status} ${resp.statusText}`,
			'warn',
		);
	} catch (e: any) {
...
async function getPackageManagerServices(id: string, token: string) {
	try {
		const select =
			'select=software,url,package_manager,download_count_scraped_at,download_count_last_error,download_count_scraping_disabled_reason,reverse_dependency_count_scraped_at,reverse_dependency_count_last_error,reverse_dependency_count_scraping_disabled_reason';
		...
		logger(
			`getPackageManagerServices...${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
			...
			getPackageManagerServices(software.id, token)
				.then(items => {
					const supported = items.filter(
						item => item.package_manager !== 'other',
					);
					if (abort === false) setServices(supported);
				})
				.catch(e => {
					logger(
						`usePackageManagerServices failed. ${e.message}`,
						'error',
					);
