...
const mockGetTestimonialsForSoftware = jest.fn(props =>
...
const mockPatchTestimonialPositions = jest.fn(props =>
	Promise.resolve([] as any),
);
jest.mock('~/utils/editTestimonial', () => ({
	getTestimonialsForSoftware: jest.fn(props =>
		mockGetTestimonialsForSoftware(props),
	),
	...
	patchTestimonialPositions: jest.fn(props =>
		mockPatchTestimonialPositions(props),
	),
}));
...
describe('frontend/components/software/edit/testimonials/index.tsx', () => {
	...
	it('can delete testimonial', async () => {
		...
		await waitFor(async () => {
			...
			const remained = await screen.findAllByTestId(
