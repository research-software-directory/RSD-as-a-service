	...
	return (
		<article className="flex flex-col bg-base-100 max-w-md">
			...
			<div className="flex flex-col p-8 gap-8 md:flex-row 2xl:flex-col">
				...
				>
					{displayName ? getDisplayInitials(person) : null}
				</Avatar>
				<div className="flex-1 flex flex-col items-start">
					<h4 className="text-primary text-2xl">
						{person?.public_orcid_profile ?
							<a
								href={`/profile/${person.public_orcid_profile}/${section}`}
							>
								{displayName}{' '}
					...
					{person?.role && <h5 className="pt-1">{person?.role}</h5>}
					{person?.affiliation && (
						<h5 className="pt-1">{person?.affiliation}</h5>
					)}
					{person?.orcid && (
						<h5 className="pt-1">
							<a
