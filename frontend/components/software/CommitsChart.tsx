...
export default function CommitsChart({
	...
	if (commit_history && Object.keys(commit_history).length > 0) {
		...
		const {lineData, lastCommitDate, totalCountY} =
			prepareDataForSoftwarePage(commit_history);
		// render
		return (
			<div className={`flex-1 w-full ${className ?? ''}`}>
				<SingleLineChart data={lineData} />
				<div className="software_commitsStat pt-4" id="commitsStat">
					<b>{totalCountY} commits</b> | Last commit{' '}
					<b>
						&#x2248;{' '}
						{getTimeAgoSince(
							new Date(),
							lastCommitDate?.toISOString() ?? null,
						)}
					</b>
				</div>
			</div>
	...
	if (typeof repository_url === 'undefined' || repository_url === null) {
		noCommitMessage =
			'We cannot display the activity graph, because we do not know the repository url.';
	} else if (
		typeof commit_history_scraped_at === 'undefined' ||
		commit_history_scraped_at === null
	) {
		// not scraped yet
		noCommitMessage =
			'We did not scrape the commit history of this repository yet.';
	} else if (
		commit_history_scraped_at &&
		commit_history &&
		Object.keys(commit_history).length == 0
	) {
		// we did scraped repo but no commit history
		noCommitMessage =
			'We cannot display this graph because the repository is empty.';
	} else if (
		commit_history_scraped_at &&
		(typeof commit_history === 'undefined' || commit_history === null)
	) {
