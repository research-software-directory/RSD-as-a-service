...
import {
	getGlobalSearch,
	GlobalSearchResults,
} from '~/components/GlobalSearchAutocomplete/globalSearchAutocomplete.api';
...
export default function GlobalSearchAutocomplete(props: Props) {
	...
	function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
		...
		switch (e.keyCode) {
			...
			case 40:
				e.preventDefault(); // Disallows the cursor to move to the end of the input
				searchResults.length - 1 > selected &&
	...
	return (
		...
		>
			...
			>
				...
				<input
					className="px-2 pl-8 py-2 bg-transparent rounded-sm border border-base-600 focus:outline-0 w-full focus:bg-base-100 focus:text-base-900 duration-200"
					ref={inputRef}
									...
									<div className="flex-grow ">
										...
										<div className="text-xs text-current text-opacity-40">
											...
											{item?.is_published === false && (
												<span className="flex-nowrap border px-1 py-[2px] rounded bg-warning ml-3 text-xs text-warning-content">
													unpublished
												</span>
											)}
										</div>
