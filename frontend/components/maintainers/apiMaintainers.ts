...
export function rawMaintainersToMaintainers(
	raw_maintainers: RawMaintainerProps[],
) {
	try {
		...
		raw_maintainers.forEach(item => {
			...
			item.name.forEach((name, pos) => {
				...
				const maintainer: MaintainerProps = {
					...
					affiliation:
						item.affiliation[pos] ? item.affiliation[pos] : '',
					// primary maintainer cannot be deleted
			...
			maintainers.push(
				maintainerWithMostInfo as unknown as MaintainerProps,
			);
		});
		return maintainers;
	} catch (e: any) {
...
export async function deleteMaintainerLink({
	invitation,
	token,
}: {
...
export async function getUnusedInvitations({
	id,
	type,
	token,
}: getUnusedInvitationsProps) {
	try {
