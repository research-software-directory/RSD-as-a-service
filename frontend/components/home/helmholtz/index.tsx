...
const SPOTLIGHTS = [
	...
	},
	{
		name: 'Golem',
		description:
			'Golem is a modelling platform for thermal-hydraulic-mechanical and non-reactive chemical processes in fractured and faulted porous media.',
	...
	function descriptionParagraph(description: string) {
		if (description != '') {
			if (description.length > MAX_CHARS) {
				...
				description =
					description_trunc.substring(
						0,
						description_trunc.lastIndexOf(' '),
					) + ' …';
			}
...
function ResearchField({background, name}: {background: string; name: string}) {
	function mouseEnter(event: React.MouseEvent<HTMLAnchorElement>) {
		...
		event.target.parentElement!.parentElement!.style.backgroundImage =
			'url("' + background + '")';
	}

	return (
		<a onMouseEnter={mouseEnter} data-background={background}>
			{name}
		</a>
	);
}
...
function ResearchFields() {
	return (
		<div
			id="researchTopicBox"
			className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-20 text-center text-3xl place-items-center py-16"
		>
			<ResearchField
				background="pexels-pixabay-414837.jpg"
				name="Energy"
			/>
			<ResearchField
				background="pexels-blue-ox-studio-695299.jpg"
				name="Earth & Environment"
			/>
			<ResearchField
				background="pexels-rfstudio-3825529.jpg"
				name="Health"
			/>
			<ResearchField
				background="jj-ying-8bghKxNU1j0-unsplash.jpg"
				name="Information"
			/>
			<ResearchField
				background="pexels-aleksejs-bergmanis-681335.jpg"
				name="Aeronautics, Space and Transport"
			/>
			<ResearchField
				background="desy_yulia-buchatskaya-hYvZHggmuc4-unsplash.jpg"
				name="Matter"
			/>
		</div>
	);
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-20">
					<div className="text-2xl">
						...
						<div className="px-6 my-4 border-l-2 border-[#002864]">
							<div className="py-4">
								Login with your Helmholtz Institution&apos;s
								account
							</div>
							<div className="py-4">
						...
						<div className="py-2">
							...
							<a
								href="mailto:support@hifis.net?subject=Comments about RSD"
								className="bg-[#cdeefb] underline"
							>
								e-mail
							</a>
							, or open an{' '}
							<a
								href="https://github.com/hifis-net/RSD-as-a-service/issues"
								target="_blank"
								className="bg-[#cdeefb] underline"
								rel="noreferrer"
							>
