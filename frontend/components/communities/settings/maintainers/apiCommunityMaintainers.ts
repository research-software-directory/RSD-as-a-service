...
export async function getMaintainersOfCommunity({
	community,
	token,
}: {
	community: string;
	...
	try {
		...
		// ERRORS
		logger(
			`getMaintainersOfCommunity: ${resp.status}:${resp.statusText} community: ${community}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function deleteMaintainerFromCommunity({
	maintainer,
	community,
	token,
}: {
...
export async function communityMaintainerLink({
	community,
	account,
	token,
}: {
