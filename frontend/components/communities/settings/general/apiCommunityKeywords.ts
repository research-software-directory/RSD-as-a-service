...
export async function getKeywordsByCommunity(uuid: string, token?: string) {
	try {
		...
		logger(
			`getKeywordsByCommunity ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function addKeywordsToCommunity({
	data,
	token,
}: {
	data: CommunityKeyword | CommunityKeyword[];
	token: string;
}) {
	try {
		// POST
...
export async function deleteKeywordFromCommunity({
	community,
	keyword,
	token,
}: {
