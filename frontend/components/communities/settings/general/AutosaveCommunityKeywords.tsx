...
import {
	KeywordForCommunity,
	addKeywordsToCommunity,
	deleteKeywordFromCommunity,
...
export default function AutosaveCommunityKeywords() {
	...
	async function onAdd(selected: Keyword) {
		...
		const find = keywords.filter(
			item =>
				item.keyword.trim().toLowerCase() ===
				selected.keyword.trim().toLowerCase(),
		);
		// debugger
		let resp;
		if (find.length === 0) {
			...
			showInfoMessage(
				`${selected.keyword.trim()} is already in the list`,
			);
		}
	}
		...
		<div className="py-8">
			...
			<div className="flex flex-wrap py-2">
				{keywords.map((item, pos) => {
					return (
						<div key={item.id} className="py-1 pr-1">
							<Chip
