...
import {
	...
	SoftwareOfCommunity,
	patchSoftwareForCommunity,
} from '../apiCommunitySoftware';
...
export function useSoftwareCardActions({software}: SoftwareCardWithMenuProps) {
	...
	async function setStatus(status: CommunityRequestStatus) {
		...
		if (resp.status !== 200) {
			showErrorMessage(
				`Failed to update ${software.brand_name}. ${resp.message}`,
			);
		} else {
	...
	function onAction(action: SoftwareMenuAction) {
		switch (action.type) {
			...
			default:
				logger(
					`Action type ${action.type} NOT SUPPORTED. Check your spelling.`,
					'warn',
				);
		}
	}
