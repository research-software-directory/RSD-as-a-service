...
} from './filters/apiCommunitySoftwareFilters';
...
export async function getSoftwareForCommunity({
	...
	try {
		...
		logger(
			`getSoftwareForCommunity: ${resp.status} ${resp.statusText}`,
			'warn',
		);
...
export async function ssrCommunitySoftwareProps({
	...
	try {
		...
		const {search, keywords, prog_lang, licenses, order, rows, page} =
			ssrSoftwareParams(query);
...
export async function patchSoftwareForCommunity({
	software,
	community,
	data,
	token,
