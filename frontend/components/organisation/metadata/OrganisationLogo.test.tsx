...
const mockDeleteImage = jest.fn(props =>
	Promise.resolve({status: 200, statusText: 'OK'}),
);
const mockUpsertImage = jest.fn(props =>
	Promise.resolve({status: 201, statusText: 'OK', message: ''}),
);
...
const mockPatchOrganisation = jest.fn(props =>
	Promise.resolve({status: 200, statusText: 'OK'}),
);
...
const mockFileUpload = jest.fn(props =>
	Promise.resolve({
		status: 200,
		message: 'OK',
...
it('can change the logo', async () => {
	...
	mockUpsertImage.mockResolvedValueOnce({
		status: 201,
		statusText: 'OK',
		message: mockProps.organisation.logo_id,
	});
