...
export default function OrganisationMetadata({
	ror_info,
}: OrganisationMetadataProps) {
	const {name, short_description, country, website, isMaintainer} =
	...
	function getAllLinks() {
		...
		if (ror_info?.addresses[0].city && ror_info?.country.country_name) {
			const query = encodeURIComponent(
				`${name},${ror_info?.addresses[0].city},${ror_info?.country.country_name}`,
			);
			...
			<BaseSurfaceRounded className="grid lg:grid-cols-[3fr,1fr] lg:gap-8 xl:grid-cols-[4fr,1fr] p-4">
				<div>
					...
					<RorLocation
						...
						country={
							ror_info?.country.country_name ?? country ?? null
						}
					/>
