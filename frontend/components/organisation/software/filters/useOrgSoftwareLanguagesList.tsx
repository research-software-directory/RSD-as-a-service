...
import {
	OrgSoftwareFilterProps,
	buildOrgSoftwareFilter,
} from './useOrgSoftwareKeywordsList';
...
export async function orgSoftwareLanguagesFilter({
	...
	try {
		...
		logger(
			`orgSoftwareLanguagesFilter: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export default function useOrgSoftwareLanguagesList() {
	...
	const {search, keywords_json, prog_lang_json, licenses_json} =
		useSoftwareParams();
	...
	}, [search, keywords_json, prog_lang_json, licenses_json, id, token]);
