...
import ViewToggleGroup, {
	ProjectLayoutType,
} from '~/components/projects/overview/search/ViewToggleGroup';
...
export default function OrgSearchSoftwareSection({
	...
	const placeholder =
		filterCnt > 0 ? 'Find within selection' : 'Find software';
		...
		<section data-testid="search-section">
			<div className="flex border rounded-md shadow-sm bg-base-100 p-2">
				<SearchInput
					...
					defaultValue={search ?? ''}
				/>
				<ViewToggleGroup layout={layout} onSetView={setView} />
				<SelectRows rows={rows} handleQueryChange={handleQueryChange} />
			</div>
			...
			{smallScreen ?
				<FiltersModal open={modal} setModal={setModal}>
					<OrgSoftwareFilters />
				</FiltersModal>
			:	undefined}
		</section>
