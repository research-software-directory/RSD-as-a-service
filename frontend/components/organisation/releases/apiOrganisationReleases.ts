...
export async function getReleasesForOrganisation({
	organisation_id,
	release_year,
	token,
	...
	try {
		...
		// some other errors
		logger(
			`getReleasesForOrganisation...${resp.status} ${resp.statusText}`,
		);
		return null;
	} catch (e: any) {
...
export async function getReleasesCountForOrganisation({
	organisation_id,
	token,
}: UseSoftwareReleaseProps) {
	try {
		...
		// some other errors
		logger(
			`getReleasesCountForOrganisation...${resp.status} ${resp.statusText}`,
		);
		return null;
	} catch (e: any) {
