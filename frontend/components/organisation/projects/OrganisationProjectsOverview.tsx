...
export default function OrganisationProjectsOverview({
	layout,
	projects,
	loading,
	rows,
}: OrganisationProjectsOverviewProps) {
		...
		return (
			<ProjectOverviewList>
				{projects.map(item => {
					if (isMaintainer) {
						return (
							<AdminProjectListItem key={item.id} item={item} />
						);
					}
					return (
	...
	if (loading === true) {
		return <GridCardSkeleton count={itemCnt} />;
	}
	...
	return (
		<ProjectOverviewGrid>
			{projects.map(item => {
				if (isMaintainer) {
					return <AdminProjectGridCard key={item.id} item={item} />;
				}
				return (
					<Link
						key={item.id}
					...
					>
						<ProjectCardContent visibleKeywords={3} {...item} />
					</Link>
				);
			})}
		</ProjectOverviewGrid>
