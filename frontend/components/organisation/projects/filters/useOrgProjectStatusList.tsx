...
export async function orgProjectStatusFilter({
	...
	try {
		...
		logger(
			`orgProjectStatusFilter: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export default function useOrgProjectStatusList() {
	...
	const {search, keywords_json, domains_json, organisations_json} =
		useProjectParams();
	...
	}, [search, keywords_json, domains_json, organisations_json, id, token]);
