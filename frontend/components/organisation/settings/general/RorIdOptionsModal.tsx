...
export default function RorIdOptionsModal({
	open,
	options,
	onSelect,
}: RorIdOptionsModalProps) {
	...
	function renderOptions() {
		return options.map(item => {
			if (item.data.ror_id) {
				return (
					<MenuItem key={item.key} value={item.data.ror_id}>
						{item.label}
					</MenuItem>
				);
			}
	...
	return (
		<Dialog disableEscapeKeyDown open={open} onClose={handleClose}>
			...
			>
				<p className="pb-8">
					Failed to find exact match on the organisation name in ROR
					database. Please select an organisation from the
					alternatives or use Cancel button to abort the process.
				</p>
