...
export async function getMaintainersOfOrganisation({
	organisation,
	token,
}: {
	organisation: string;
	...
	try {
		...
		// ERRORS
		logger(
			`getMaintainersOfOrganisation: ${resp.status}:${resp.statusText} organisation: ${organisation}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function deleteMaintainerFromOrganisation({
	maintainer,
	organisation,
	token,
}: {
...
export async function organisationMaintainerLink({
	organisation,
	account,
	token,
}: {
