...
describe('frontend/components/organisation/software/index.tsx', () => {
	...
	it('shows no units message', async () => {
		...
		const noUnitsMsg = screen.getByText(
	...
	it('primary maintainer can add research unit', async () => {
		...
		const slugInput = await within(modal).findByPlaceholderText(
			config.slug.label,
		);
		...
		mockResolvedValueOnce('OK', {
			...
			headers: {
				...
				get: (prop: string) => {
					if (prop === 'location')
						return '/organisation?id=eq.ad13eb96-dd3e-4f8b-9b9f-d63ee7a078b8';
					return prop;
				},
			},
		});
