	...
	function handleFileUpload({target}: {target: any}) {
		try {
			...
			if (file.size > 2097152) {
				...
				showWarningMessage(
					'The file is too large. Please select image < 2MB.',
				);
				return;
			}
			...
			reader.onloadend = function () {
				if (reader.result) {
					...
					replaceLogo(reader.result as string, file.type);
				}
			};
			...
			>
				...
				>
					<section className="grid grid-cols-[1fr,3fr] gap-8">
						<div>
							<label
								htmlFor="upload-avatar-image-modal"
							...
							<div className="flex pt-4">
								<Button
									...
									// color='primary'
									disabled={
										!formData.logo_b64 && !formData.logo_id
									}
									onClick={deleteLogo}
								>
									Remove
