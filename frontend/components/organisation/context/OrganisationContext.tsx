...
export function OrganisationProvider(props: any) {
	...
	const {organisation: initOrganisation, isMaintainer: initMaintainer} =
		props;
	// set state - use initOrganisation at start
	const [organisation, setOrganisation] =
		useState<OrganisationForOverview | null>(initOrganisation);
	const [isMaintainer, setIsMaintainer] = useState<boolean>(
		initMaintainer ?? false,
	);

	const updateOrganisation = useCallback(
		({key, value}: UpdateOrganisationProps) => {
			if (organisation) {
				const org = {
					...organisation,
					[key]: value,
