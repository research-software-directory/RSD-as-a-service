 ...
 */
export function useAdvicedDimensions(
	source: 'software' | 'project' | 'organisation' = 'project',
) {
	...
	if (source === 'software' && itemHeight === '26rem') {
...
export const FlexibleGridSection = styled('section', {
	...
	if (height) {
		...
		props['gridTemplateRows'] =
			`repeat(auto-fit,minmax(${minHeight ?? '5rem'}, ${maxHeight ?? '7rem'}))`;
	}
	// dynamic grid-template-columns
	if (minWidth && maxWidth) {
		props['gridTemplateColumns'] =
			`repeat(auto-fit, minmax(${minWidth}, ${maxWidth ?? '7rem'}))`;
	}
	return props;
});
