...
realLabels.set('date_start', {
	label: 'Start date',
	type: 'text',
	sx: {whiteSpace: 'nowrap'},
});
realLabels.set('date_end', {
	label: 'End date',
	type: 'text',
	sx: {whiteSpace: 'nowrap'},
});
...
realLabels.set('has_contact_person', {
	label: 'Contact person',
	type: 'boolean',
});
realLabels.set('team_member_cnt', {label: 'Team members', type: 'number'});
realLabels.set('participating_org_cnt', {
	label: 'Participating organisations',
	type: 'number',
});
realLabels.set('funding_org_cnt', {
...
realLabels.set('research_domain_cnt', {
...
export function useProjectQuality({
	token,
	showAll,
}: {
	token: string;
	...
	useEffect(() => {
		if (token) {
			setDataLoaded(false);
			fetchProjectQuality(showAll, token).then(data => {
				setData(data);
				setDataLoaded(true);
			});
		}
