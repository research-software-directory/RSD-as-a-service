...
export default function SortableTable({
							...
							{propKeys.map(metadataKey => {
								return (
									...
									>
										{renderCell(
											item,
											metadataKey as ProjectQualityKeys,
										)}
									</TableCell>
								);
							})}
	...
	function renderCell(
		item: ProjectQualityProps,
		metadataKey: ProjectQualityKeys,
	) {
		...
		switch (metaData.type) {
			...
			case 'number':
				return (item[metadataKey] as number) > 0 ?
						item[metadataKey]
					:	<CloseIcon sx={{color: 'error.main'}} />;
			case 'link':
	...
	function sortByLabel(label: string) {
		...
		if (type === 'boolean' || type === 'number' || type === 'pct') {
			data.sort((a, b) =>
				nowAscending ? a[label] - b[label] : b[label] - a[label],
			);
		} else {
			data.sort((a, b) =>
				compareStringNullsAware(a[label], b[label], nowAscending),
			);
		}
	...
	function compareStringNullsAware(
		s1: string | null,
		s2: string | null,
		ascending: boolean,
	): number {
