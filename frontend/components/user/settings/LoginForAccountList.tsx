...
function LoginAccount({
	...
	const disabled =
		account.provider === 'orcid' && public_orcid_profile === true;
	...
	return (
		...
		>
			...
			<ListItemText
				...
				secondary={
					<>
						<span data-testid="user-settings-username">
							{account?.name ?? ''}
						</span>
						...
						{account?.provider === 'orcid' ?
							<>
								<br />
								<OrcidLink orcid={account?.sub} />
							</>
						:	null}
						{account?.home_organisation ?
							<>
								<br />
								<span data-testid="affiliation">
									{account?.home_organisation}
								</span>
							</>
						:	null}
...
export default function LoginForAccountList({
	accounts,
	deleteLogin,
}: LoginForAccountListProps) {
			...
			<ConfirmDeleteModal
				...
				body={
					<p>
						...
						<strong>
							{modal?.account?.provider} authentication method
						</strong>
						?
					</p>
				}
