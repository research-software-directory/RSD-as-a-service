...
export default function UserAgreementModal() {
	...
	const {loading, agree_terms, notice_privacy_statement} =
		useUserAgreements();
	const [open, setOpen] = useState<boolean>(false);
	const {handleSubmit, register, control, watch} =
		useForm<UserSettingsModalForm>({
			mode: 'onChange',
			defaultValues: {
	...
	const [agreeTerms, privacyStatement] = watch([
		'agree_terms',
		'notice_privacy_statement',
	]);
		...
		patchAccountTable({
			...
			.then(resp => {
				if (resp?.status === 200) {
					...
					showErrorMessage(
						`Failed to restore agreements. ${resp?.message}`,
					);
				}
			})
			.catch(err => {
				showErrorMessage(
					`Failed to restore agreements. ${err?.message}`,
				);
			});
	}
		...
		>
			...
			>
				...
				>
					...
					<div className="py-4">
						To be able to contribute to the RSD, we need to know
						that you agree to our Terms of Service, and that you
						have read the Privacy Statement. Please check all of the
						points below to proceed:
					</div>
					<div>
						<ControlledSwitch
					...
					<p className="py-4">
						You may view or modify your agreement at any time in
