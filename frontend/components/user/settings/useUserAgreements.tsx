...
export async function getUserAgreements(token: string, account: string) {
	const select = 'agree_terms,notice_privacy_statement,public_orcid_profile';
...
export async function patchAccountTable({
	account,
	data,
	token,
}: PatchAccountTableProps) {
	try {
...
export function useUserAgreements() {
	...
	useEffect(() => {
		...
		async function getAgreements() {
			setLoading(true);
			const settings = await getUserAgreements(
				token,
				user?.account ?? '',
			);
	...
	const setAgreeTerms = useCallback(
		async (agree_terms: boolean) => {
			if (user?.account) {
				...
				if (resp.status !== 200) {
					showErrorMessage(
						`Failed to update account. ${resp.message}`,
					);
				} else {
	...
	const setPrivacyStatement = useCallback(
		async (notice_privacy_statement: boolean) => {
			if (user?.account) {
				const resp = await patchAccountTable({
					account: user?.account,
					token,
	...
	const setPublicOrcidProfile = useCallback(
		async (public_orcid_profile: boolean) => {
			if (user?.account) {
				const resp = await patchAccountTable({
					account: user?.account,
					token,
