...
export default function ProgrammingLanguagesFilter({
	prog_lang,
	languagesList,
	handleQueryChange,
	...
	const [options, setOptions] =
		useState<LanguagesFilterOption[]>(languagesList);
	...
	return (
		<>
			<FilterTitle title={title} count={languagesList.length ?? ''} />
			<Autocomplete
				className="mt-4"
				value={selected}
				size="small"
				...
				onChange={(_, newValue) => {
					...
					const queryFilter = newValue.map(
						item => item.prog_language,
					);
					// update query url
