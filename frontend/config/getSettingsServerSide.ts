...
export async function getRsdSettings() {
	try {
		...
		if (resp.status === 200) {
			...
			logger(
				`Failed to load theme settings. ${resp.status} ${resp.statusText}`,
				'warn',
			);
...
export async function getSettingsServerSide(
	req: IncomingMessage | undefined,
	query: ParsedUrlQuery,
): Promise<RsdSettingsState> {
	// if not SSR we return default
	if (typeof req === 'undefined')
		return defaultRsdSettings as RsdSettingsState;
	...
	if (!settings.host.software_highlights) {
		settings.host.software_highlights =
			defaultSettings.host.software_highlights;
	}
