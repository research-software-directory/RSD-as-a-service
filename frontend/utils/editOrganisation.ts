...
export async function searchForOrganisation({
	...
	try {
		...
		const rorIdsFound: string[] = rorOptions.map(
			rorResult => rorResult.key,
		);
		const rsdOptions = await findRSDOrganisation({
			searchFor,
			token,
			rorIds: rorIdsFound,
		});
...
export async function findRSDOrganisation({
	searchFor,
	token,
	rorIds,
}: {
	...
	try {
		...
		if (resp.status === 200) {
			...
			const options: AutocompleteOption<SearchOrganisation>[] = data.map(
				item => {
					return {
		...
		logger(
			`findRSDOrganisation ERROR: ${resp?.status} ${resp?.statusText}`,
			'error',
		);
		return [];
	} catch (e: any) {
...
export async function getOrganisationsForSoftware({
	software,
	token,
	frontend = true,
}: {
...
export async function getParticipatingOrganisations({
	software,
	token,
	frontend = true,
}: {
...
export async function createOrganisation({
	...
	organisation: CoreOrganisationProps;
	token: string;
}) {
	try {
...
export async function getRsdPathForOrganisation({
	uuid,
	token,
}: {
	uuid: string;
...
export function searchToEditOrganisation({
	item,
	account,
	position,
}: {
	item: SearchOrganisation;
	account?: string;
	position?: number;
}) {
