...
import {
	DataciteWorkGraphQLResponse,
	DataciteWorksGraphQLResponse,
	WorkResponse,
} from '~/types/Datacite';
...
export async function getDataciteItemsByTitleGraphQL(title: string) {
	try {
		...
		if (resp.status === 200) {
			...
			if (json.data.works && json.data.works.nodes)
				return json.data.works.nodes;
			return [];
		}
		logger(
			`getDataciteItemsByTitleGraphQL: ${resp.status}: ${resp?.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
