...
export async function getDoiInfo(doiId: string) {
	try {
		...
		logger(
			`getting DOI info FAILED: ${resp.status}: ${resp.statusText}`,
			'warn',
		);
		return null;
	} catch (e: any) {
...
export async function getContributorsFromDoi(
	...
	allPersons.forEach(person => {
		...
		if (person?.nameIdentifiers?.length > 0) {
			person?.nameIdentifiers.forEach(id => {
				if (id.nameIdentifierScheme === 'ORCID') {
					if (
						id.nameIdentifier.startsWith('https://orcid.org/') ===
						true
					) {
						orcid = id.nameIdentifier.replace(
							'https://orcid.org/',
							'',
						);
					} else {
		...
		if (
			person.hasOwnProperty('givenName') === true &&
			person.hasOwnProperty('familyName') === true
		) {
