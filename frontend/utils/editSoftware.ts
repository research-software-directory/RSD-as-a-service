...
export async function getSoftwareToEdit({
	...
	try {
		// GET
		const select =
			'*,repository_url!left(url,code_platform,scraping_disabled_reason)';
		...
		if (resp.status === 200) {
			...
			// fix repositoryUrl
			const software: SoftwareItem = getPropsFromObject(
				data[0],
				SoftwarePropsToSave,
			);
			...
			if (data[0]?.repository_url?.url?.length > 9) {
				...
				software.repository_platform =
					data[0]?.repository_url?.code_platform;
			} else {
				software.repository_url = null;
				software.repository_platform = null;
			}
			software.scraping_disabled_reason =
				data[0]?.repository_url?.scraping_disabled_reason;
			return software;
		}
	} catch (e: any) {
...
export async function addToRepositoryTable({
	data,
	token,
}: {
	data: RepositoryUrl;
...
export async function deleteFromRepositoryTable({
	software,
	token,
}: {
	software: string;
...
export async function addLicensesForSoftware({
	license,
	token,
}: {
	license: LicenseForSoftware;
	token: string;
}) {
	try {
...
export async function deleteLicense({id, token}: {id: string; token: string}) {
	try {
...
export async function validSoftwareItem(
...
export async function addRelatedProjects({
	origin,
	relation,
	token,
}: {
