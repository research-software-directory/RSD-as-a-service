...
export async function silentKeywordDelete({
	keyword,
	token,
}: {
	keyword: string;
...
export async function deleteKeywordFromProject({
	project,
	keyword,
	token,
}: {
...
export async function addKeywordsToSoftware({
	data,
	token,
}: {
	data: SoftwareKeyword | SoftwareKeyword[];
	token: string;
}) {
	try {
		// POST
...
export async function deleteKeywordFromSoftware({
	software,
	keyword,
	token,
}: {
