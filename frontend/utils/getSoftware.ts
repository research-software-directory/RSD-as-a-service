...
export async function getSoftwareList({
	...
	try {
		...
		if ([200, 206].includes(resp.status)) {
			...
			logger(
				`getSoftwareList failed: ${resp.status} ${resp.statusText} ${url}`,
				'warn',
			);
			return {
				count: 0,
...
export async function getRepositoryInfoForSoftware(
	...
	try {
		...
		if (resp.status === 200) {
			...
			if (data.length === 1 && data[0]?.release?.mention) {
				const releases: SoftwareVersion[] =
					data[0]['release']['mention'];
				return releases;
			}
			return null;
...
export async function getCategoriesForSoftware(
	...
	try {
		const url = prepareQueryURL(
			'/rpc/category_paths_by_software_expanded',
			{software_id},
		);
		const resp = await fetch(url, {
			method: 'GET',
		...
		if (resp.status === 200) {
			...
			logger(
				`getCategoriesForSoftware response: ${JSON.stringify(data)}`,
			);
			return data;
		} else if (resp.status === 404) {
...
export async function getCategoryForSoftwareIds(
	software_id: string,
	token?: string,
): Promise<CategoryForSoftwareIds> {
	try {
		const url = prepareQueryURL('/category_for_software', {
			software_id: `eq.${software_id}`,
			select: 'category_id',
		});
		const resp = await fetch(url, {
			method: 'GET',
		...
		if (resp.status === 200) {
			const data = await resp.json();
			logger(
				`getCategoriesForSoftwareIds response: ${JSON.stringify(data)}`,
			);
			...
			logger(
				`getCategoriesForSoftwareIds: ${resp.status} [${url}]`,
				'error',
			);
...
export async function deleteCategoryToSoftware(
	...
	const url = prepareQueryURL(
		`/category_for_software?software_id=eq.${softwareId}&category_id=eq.${categoryId}`,
	);
...
export async function getCommunitiesOfSoftware({
	software,
	token,
	...
	try {
		...
		logger(
			`getCommunitiesOfSoftware: ${resp.status}:${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
