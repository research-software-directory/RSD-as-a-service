...
export function decodeJsonParam(value: string | null, defaultValue: any) {
	try {
		...
		// debugger
		logger(
			`decodeJsonParam: failed to decode ${value} ${e.message ?? ''}`,
			'error',
		);
		return defaultValue;
	}
...
export function decodeQueryParam({
	...
	param: string;
	castToType?: 'string' | 'number' | 'json-encoded';
	defaultValue: any;
}) {
	try {
		if (query && query.hasOwnProperty(param)) {
			...
			switch (castToType) {
				case 'number':
					...
					logger(
						`decodeQueryParam: query param ${param} NOT a string. Returning defaultValue`,
						'warn',
					);
					return defaultValue;
				case 'string':
				...
				default:
					logger(
						`decodeQueryParam: castToType ${castToType} NOT supported. Returning defaultValue`,
						'warn',
					);
					return rawVal;
			}
