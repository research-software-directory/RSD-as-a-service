...
export async function getProjectList({
	...
	try {
		...
		if ([200, 206].includes(resp.status)) {
			...
			logger(
				`getProjectList failed: ${resp.status} ${resp.statusText} ${url}`,
				'warn',
			);
			return {
				count: 0,
...
export async function getOrganisationsOfProject({
	project,
	token,
	frontend = true,
	roles,
	...
	try {
		...
		logger(
			`getOrganisationsOfProject: ${resp.status} ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function getResearchDomainsForProject({
	project,
	token,
	frontend = false,
}: {
	...
	try {
		...
		logger(
			`getKeywordsForProject: [${resp.status}] ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function getKeywordsForProject({
	project,
	token,
	frontend = false,
}: {
...
export async function getLinksForProject({
	...
	try {
		...
		logger(
			`getLinksForProject: [${resp.status}] ${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
...
export async function getMentionsForProject({
	project,
	token,
	table,
}: {
...
export async function getImpactByProject({
	project,
	token,
}: {
	project: string;
...
export async function getTeamForProject({
	project,
	token,
}: {
	project: string;
	...
	try {
		...
		logger(
			`getTeamForProject: ${resp.status} ${resp.statusText} [${url}]`,
			'warn',
		);
		// / query not found
		return [];
...
export async function getRelatedProjectsForProject({
	project,
	token,
	frontend,
	approved = true,
	...
	try {
		...
		logger(
			`getRelatedProjects: ${resp.status} ${resp.statusText} [${url}]`,
			'warn',
		);
		// query not found
		return [];
...
export async function getRelatedSoftwareForProject({
	project,
	token,
	frontend,
	approved = true,
	...
	try {
		...
		logger(
			`getRelatedSoftwareForProject: ${resp.status} ${resp.statusText} [${url}]`,
			'warn',
		);
		// query not found
		return [];
...
export async function searchForRelatedProjectByTitle({
	project,
	searchFor,
	token,
}: {
	...
	try {
		...
		logger(
			`searchForRelatedProjectByTitle: ${resp.status} ${resp.statusText} [${url}]`,
			'warn',
		);
		return [];
	} catch (e: any) {
