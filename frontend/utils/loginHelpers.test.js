...
import {getRedirectUrl, getEncodedClaims, claims} from './loginHelpers'
...
it('returns authorization_endpoint from wellknown for SURFconext', async () => {
  ...
  mockResolvedValueOnce({
    ...
    env: {
      ...
      NEXT_PUBLIC_SURFCONEXT_RESPONSE_MODE: process.env.NEXT_PUBLIC_SURFCONEXT_RESPONSE_MODE,
      NEXT_PUBLIC_SURFCONEXT_SCOPES: process.env.NEXT_PUBLIC_SURFCONEXT_SCOPES
    }
  })
  ...
  const expected = mockResp['authorization_endpoint'] +
    '?redirect_uri=' + process.env.NEXT_PUBLIC_SURFCONEXT_REDIRECT +
    '&client_id=' + process.env.NEXT_PUBLIC_SURFCONEXT_CLIENT_ID +
    '&scope=' + process.env.NEXT_PUBLIC_SURFCONEXT_SCOPES +
    '&response_type=code&response_mode=' + process.env.NEXT_PUBLIC_SURFCONEXT_RESPONSE_MODE +
    '&prompt=login+consent&claims=' + urlEncodedClaims
  expect(redirectUrl).toEqual(expected)
})

it('returns authorization_endpoint from wellknown for Helmholtz AAI', async () => {
  // mock provider api call
  mockResolvedValueOnce({
  ...
  // then mock return value from fetch
  mockResolvedValueOnce(mockResp)
  const redirectUrl = await getRedirectUrl('helmholtzaai')
  const urlEncodedClaims = getEncodedClaims(claims)
  const expected = mockResp['authorization_endpoint'] +
    '?redirect_uri=' + process.env.NEXT_PUBLIC_HELMHOLTZAAI_REDIRECT +
    '&client_id=' + process.env.NEXT_PUBLIC_HELMHOLTZAAI_CLIENT_ID +
    '&scope=' + process.env.NEXT_PUBLIC_HELMHOLTZAAI_SCOPES +
    '&response_type=code&response_mode=' + process.env.NEXT_PUBLIC_HELMHOLTZAAI_RESPONSE_MODE +
    '&prompt=login+consent&claims=' + urlEncodedClaims
  expect(redirectUrl).toEqual(expected)
})
