...
export async function getRelatedSoftwareForSoftware({
	software,
	token,
	frontend,
}: {
	...
	try {
		...
		logger(
			`getRelatedSoftwareForSoftware: ${resp.status} ${resp.statusText} [${url}]`,
			'error',
		);
		// query not found
		return [];
...
export async function searchForRelatedSoftware({
	software,
	searchFor,
	token,
}: {
