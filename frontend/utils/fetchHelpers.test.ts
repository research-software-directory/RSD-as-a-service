...
it('extracts 4xx response message', async () => {
	...
	for (const status of statuses) {
		const resp: any = {
			...
			json: () =>
				Promise.resolve({message: `Custom message for ${status}`}),
		};
...
it('promiseWithTimeout resolves promise', async () => {
	...
	const resp = await promiseWithTimeout(mockPromise(1), 2);
...
it('promiseWithTimeout resoves request timeout', async () => {
	...
	// should resolve timeout
	try {
		const resp = await promiseWithTimeout(mockPromise(2), 1);
