...
describe('sortFn', () => {
	...
	it('order desc on prop name', () => {
		const resp = toOrder.sort((a, b) =>
			sortOnStrProp(a, b, 'name', 'desc'),
		);
		const first = resp[0];
		const last = resp[resp.length - 1];
	...
	it('order desc op date', () => {
		const resp = orderAsc.sort((a, b) =>
			sortOnDateProp(a, b, 'datum', 'desc'),
		);
		const first = resp[0];
		const last = resp[resp.length - 1];
	...
	it('order by searchFor', () => {
		const resp = orderSearchFor.sort((a, b) =>
			sortBySearchFor(a, b, 'name', 'cd'),
		);
		const first = resp[0];
		const second = resp[1];
		const third = resp[2];
