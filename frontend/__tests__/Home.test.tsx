...
describe('pages/index.tsx', () => {
	it('renders default RSD Home page when host=rsd', () => {
		render(
			WrappedComponentWithProps(Home, {
				props,
			}),
		);
	...
	it('renders counts on RSD Home page', () => {
		...
		// software_cnt
		const software = screen.getByText(
			`${props.counts.software_cnt} Software`,
		);
		expect(software).toBeInTheDocument();
		// project_cnt
		const project = screen.getByText(
			`${props.counts.project_cnt} Projects`,
		);
		expect(project).toBeInTheDocument();
		// organisation_cnt
		const organisation = screen.getByText(
			`${props.counts.organisation_cnt} Organisations`,
		);
