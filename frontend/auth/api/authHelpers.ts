...
export async function getAuthorisationEndpoint(wellknownUrl: string) {
	...
	if (resp.status === 200) {
		...
		if (data['authorization_endpoint']) {
			...
			logger(
				'getAuthorisationEndpoint failed: authorization_endpoint property MISSING',
				'error',
			);
			// return null
		}
...
export function getRedirectUrl(props: RedirectToProps) {
	let redirectUrl =
		...
		props.response_mode +
		'&prompt=' +
		(props.prompt ? props.prompt : 'login+consent');

	if (props?.claims) {
		redirectUrl +=
			'&claims=' + encodeURIComponent(JSON.stringify(props.claims));
	}

	return redirectUrl;
}

export async function claimProjectMaintainerInvite({
	id,
	token,
	frontend = false,
}: {
	...
	try {
		...
		logger(
			`claimProjectMaintainerInvite failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
...
export async function claimSoftwareMaintainerInvite({
	id,
	token,
	frontend = false,
}: {
	...
	try {
		...
		logger(
			`claimSoftwareMaintainerInvite failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		const error = await extractReturnMessage(resp);
		return {
...
export async function claimOrganisationMaintainerInvite({
	id,
	token,
	frontend = false,
}: {
	...
	try {
		...
		logger(
			`claimOrganisationMaintainerInvite failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		const error = await extractReturnMessage(resp);
		return {
		...
		logger(
			`claimOrganisationMaintainerInvite failed: ${e?.message}`,
			'error',
		);
		return {
			organisationInfo: null,
...
export async function claimCommunityMaintainerInvite({
	id,
	token,
}: {
	id: string;
	...
	try {
		...
		logger(
			`claimCommunityMaintainerInvite failed: ${resp?.status} ${resp.statusText}`,
			'error',
		);
		const error = await extractReturnMessage(resp);
		return {
