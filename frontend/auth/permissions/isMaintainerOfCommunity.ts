...
export async function isCommunityMaintainer({
	community,
	role,
	account,
	token,
}: isCommunityMaintainerProps) {
...
export async function isMaintainerOfCommunity({
	community,
	account,
	token,
}: isCommunityMaintainerProps) {
...
export async function getCommunitiesOfMaintainer({token}: {token: string}) {
	try {
		...
		// ERRORS AS NOT MAINTAINER
		logger(
			`getCommunitiesOfMaintainer: ${resp.status}:${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
