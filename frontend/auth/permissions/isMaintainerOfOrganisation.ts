...
export async function isOrganisationMaintainer({
	organisation,
	role,
	account,
	token,
}: IsOrganisationMaintainerProps) {
...
export async function isMaintainerOfOrganisation({
	organisation,
	account,
	token,
}: IsMaintainerOfOrganisationProps) {
	try {
		if (typeof account == 'undefined' || typeof token == 'undefined') {
...
export async function getMaintainerOrganisations({token}: {token: string}) {
	try {
		...
		// ERRORS AS NOT MAINTAINER
		logger(
			`getMaintainerOrganisations: ${resp.status}:${resp.statusText}`,
			'warn',
		);
		return [];
	} catch (e: any) {
