...
def get_maintainer_emails_for_community(community_id):
    ...
    return response.json()[0]["email"] #[maintainer['email_address'] for maintainer in response.json()], utils.get_common_value(response.json(), 'name'), utils.get_common_value(response.json(), 'slug')
