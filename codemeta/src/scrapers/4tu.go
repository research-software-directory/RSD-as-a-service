...
func main() {
	...
	const url4tu = "https://data.4tu.nl/v3/codemeta?limit=1000"
...
func extractAndEscapeShortStatement(description terms.SingleOrArray[string]) *string {
	if description.Single != nil {
		return nil
	} else if description.Array != nil {
		var array []string = description.Array
		if len(array) >= 2 {
			result := html.EscapeString(array[0])
			return &result
		}
	}
...
func extractAndEscapeDescription(description terms.SingleOrArray[string]) *string {
	if description.Single != nil {
		...
		var array []string = description.Array
		if len(array) >= 2 {
			result := html.EscapeString(array[1])
			return &result
		} else if len(array) == 1 {
			result := html.EscapeString(array[0])
			return &result
...
func saveApplicationsInRsd(softwareSlice []terms.SoftwareApplication, adminJwt string, backendUrl string) error {
	...
	for _, software := range softwareSlice {
		rsdSoftware := RsdSoftwareTable{
			...
			ShortStatement: extractAndEscapeShortStatement(software.Description),
		}
...
func saveKeywordsForSoftware(id string, software terms.SoftwareApplication, adminJwt string, backendUrl string) error {
	var err error
	//TODO: maybe remove double spaces in the keywords
...
func saveReferencePublicationsForSoftware(softwareId string, software terms.SoftwareApplication, adminJwt string, backendUrl string) error {
	for _, article := range software.ReferencePublication {
		...
		var doi string = *article.Id
		if !doiPattern.MatchString(doi) {
			fmt.Printf("Invalid DOI: %s\n", doi)
			return nil
		}

		request, err := http.NewRequest("GET", backendUrl+"/mention?select=id&doi=eq."+doi, nil)
		if err != nil {
			return err
		}
