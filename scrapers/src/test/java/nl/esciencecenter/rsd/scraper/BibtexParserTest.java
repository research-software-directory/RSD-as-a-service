...
public class BibtexParserTest {
	...
	void givenValidBibtex_whenParsing_thenParsedCorrectly() throws InvalidInputException, IOException {
		...
		Map<String, MentionRecord> result = BibtexParser.parse(new StringReader(validBibtex));

		Assertions.assertEquals(3, result.size());
		Assertions.assertEquals("Brughmans, Tom", result.get("Brughmans2018").authors);
		Assertions.assertEquals("Network structures and assembling code in Netlogo, Tutorial", result.get("Brughmans2018").title);
	...
	void givenValidBibtex_whenParsing_thenParsedCorrectly2() throws InvalidInputException, IOException {
		...
		Assertions.assertEquals("Agent-Based Modeling for Archaeology", result.get("romanowska_agent-based_2021").title);
		Assertions.assertEquals("10.37911/9781947864382", result.get("romanowska_agent-based_2021").doi);
