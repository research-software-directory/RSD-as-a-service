...
class RorPostgrestConnectorTest {
	...
	void testParseBasicJsonData() {
		...
		Collection<BasicOrganisationData> result = RorPostgrestConnector.parseBasicJsonData(data);
		assertEquals(1, result.size());

		BasicOrganisationData organisation = result.iterator().next();
		assertEquals(UUID.fromString("52b13a36-334b-429a-9cbb-5215264b36d2"), organisation.id());
		assertEquals(null, organisation.city());
		assertEquals(null, organisation.country());
	}
}
