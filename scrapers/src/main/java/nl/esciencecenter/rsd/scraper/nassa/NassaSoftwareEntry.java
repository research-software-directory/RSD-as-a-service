...
public class NassaSoftwareEntry {
	...
	public static NassaSoftwareEntry fromYaml(Reader yamlSource, String gitHtmlUrl) {
		Yaml yaml = new Yaml();
		...
		for (Map<String, Object> contributorMap : contributorsUnparsed) {
			contributors.add(NassaContributor.fromParsedYaml(contributorMap));
		}
	...
	public String toRsdJson(Map<String, MentionRecord> mentions) {
		...
		root.addProperty("short_statement", description.length() > 300 ? description.substring(0, 297) + "..." : description);
		...
		if (license != null) {
			switch (license) {
				case "MIT" -> {
					root.addProperty("license_value", "MIT");
					root.addProperty("license_name", "MIT License");
					root.addProperty("license_url", "https://spdx.org/licenses/MIT.html");
					root.addProperty("license_open_source", true);
				}
				case "CC BY-SA 4.0" -> {
					root.addProperty("license_value", "CC-BY-SA-4.0");
					root.addProperty("license_name", "Creative Commons Attribution Share Alike 4.0 International");
					root.addProperty("license_url", "https://spdx.org/licenses/CC-BY-SA-4.0.html");
					root.addProperty("license_open_source", true);
				}
				default -> LOGGER.warn("Unexpected license: {}", license);
			}
		}
		...
		JsonArray familyNamesArray = new JsonArray();
		JsonArray givenNamesArray = new JsonArray();
		JsonArray roleArray = new JsonArray();
		JsonArray orcidArray = new JsonArray();
		...
		Collection<MentionRecord> regularMentions = new ArrayList<>();
...
class NassaContributor {
	...
	static NassaContributor fromParsedYaml(Map<String, Object> map) {
		String name = (String) map.get("name");
		String[] nameSplitted = name.split(", ");
		String givenName = nameSplitted.length >= 2 ? nameSplitted[1] : "";
		String familyName = nameSplitted[0];
		@SuppressWarnings("unchecked")
