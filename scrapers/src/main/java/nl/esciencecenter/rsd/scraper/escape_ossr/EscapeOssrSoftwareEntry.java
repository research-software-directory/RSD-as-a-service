...
import nl.esciencecenter.rsd.scraper.git.GithubScraper;
import nl.esciencecenter.rsd.scraper.git.GitlabScraper;
...
public class EscapeOssrSoftwareEntry {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(EscapeOssrSoftwareEntry.class);
	...
	Optional<String> descriptionUrl = Optional.empty();
	final Optional<String> getStartedUrl;
	final String repoUrl;
	final CodePlatformProvider codePlatform;
	final List<String> keywords;
	final List<RsdContributor> contributors;
	final Optional<String> license;
	final List<String> applicationCategories;
	final List<String> applicationSubCategories;
	...
	private EscapeOssrSoftwareEntry(
		...
		List<String> applicationSubCategories
	) {
		this.name = name;
	...
	public static EscapeOssrSoftwareEntry fromCodemeta(String codemeta, Doi conceptDoi) throws RsdParsingException {
		try {
			...
			try {
				shortDescription = HtmlToMarkdown.convert(
					description,
					ConversionOptions.builder().withOutputFormat(OutputFormat.Plain).withSkipImages(true).build()
				).content();
			} catch (Exception e) {
				LOGGER.warn("Could not convert description as HTML to plaintext", e);
			}
			String longDescription = description;
			try {
				longDescription = HtmlToMarkdown.convert(
					description,
					ConversionOptions.builder().withOutputFormat(OutputFormat.Markdown).build()
				).content();
			} catch (Exception e) {
				LOGGER.warn("Could not convert description as HTML to Markdown", e);
			}

			Optional<String> getStartedUrl = Optional.empty();
			...
			if (readmeJson != null && readmeJson.isJsonPrimitive()) {
				...
				if (Utils.isUrl(readmeUrlToCheck)) {
					getStartedUrl = Optional.of(readmeUrlToCheck);
				}
			}
			...
			contributors.addAll(parseContributorArray(root.get("author"), "author", 1));
			contributors.addAll(parseContributorArray(root.get("contributor"), "contributor", contributors.size() + 1));
			contributors.addAll(parseContributorArray(root.get("maintainer"), "maintainer", contributors.size() + 1));
			...
			JsonElement applicationCategoriesJson = root.get("applicationCategory");
			if (applicationCategoriesJson != null && applicationCategoriesJson.isJsonPrimitive()) {
				applicationCategories.addAll(Arrays.asList(applicationCategoriesJson.getAsString().split(", ")));
			}

			List<String> applicationSubCategories = new ArrayList<>();
			JsonElement applicationSubCategoriesJson = root.get("applicationSubCategory");
			if (applicationSubCategoriesJson != null && applicationSubCategoriesJson.isJsonPrimitive()) {
				applicationCategories.addAll(Arrays.asList(applicationSubCategoriesJson.getAsString().split(", ")));
			}
	...
	/**
	 *
	 * Retrieve the raw README URL of the repository and store it in this object, overwriting the previous result (if it existed).
	 *
	 * @throws RsdResponseException when the HTTP request to the API of the repo gives an unexpected response
	 * @throws IOException when the HTTP request to the API throws this
	 * @throws InterruptedException when the HTTP request to the API is interrupted
	 */
	public void harvestReadmeUrl() throws RsdResponseException, IOException, InterruptedException {
		Optional<URI> rawReadmeUrl = switch (codePlatform) {
			case GITHUB -> {
				Optional<GithubScraper> githubScraper = GithubScraper.create(repoUrl);
				if (githubScraper.isEmpty()) {
					yield Optional.empty();
				}

				yield githubScraper.get().rawReadmeUrl();
			}
			case GITLAB -> {
				...
				GitlabScraper gitScraper = new GitlabScraper("https://" + uri.getHost() + "/api", path);
				yield gitScraper.rawReadmeUrl();
			}
			case null, default -> Optional.empty();
		};

		this.descriptionUrl = rawReadmeUrl.map(URI::toASCIIString).filter(url -> url.endsWith(".md"));
	}

	private static List<RsdContributor> parseContributorArray(JsonElement element, String role, int startPosition) {
		...
		for (JsonElement jsonElement : array) {
			...
			RsdContributor contributor = new RsdContributor(
				givenNames,
				familyNames,
				affiliation,
				Optional.of(role),
	...
	public String toRsdJson(Map<String, SpdxLicense> licenseMap) {
		...
		root.addProperty("slug", "escape-ossr-" + Utils.slugify(name));
		...
		root.addProperty("description", longDescription);
		root.addProperty("description_url", descriptionUrl.orElse(null));
		root.addProperty("get_started_url", getStartedUrl.orElse(null));
		...
		for (RsdContributor contributor : contributors) {
			...
			roleArray.add(contributor.role().orElseThrow());
		...
		root.add("categories", categoryObject);

		JsonArray applicationCategoryValues = new JsonArray();
		applicationCategories.forEach(applicationCategoryValues::add);
		categoryObject.add("Application category", applicationCategoryValues);

		JsonArray applicationSubCategoryValues = new JsonArray();
		applicationSubCategories.forEach(applicationSubCategoryValues::add);
		categoryObject.add("Application sub category", applicationSubCategoryValues);
	...
	static Optional<SpdxLicense> extractSpdxLicense(String rawLicense, Map<String, SpdxLicense> licenseMap) {
		if (rawLicense == null) {
			return Optional.empty();
		}
